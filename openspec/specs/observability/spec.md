...
#### Scenario: Profile-update controller diagnostics exclude request and account values
- **WHEN** the profile-update controller receives a request, rejects its DTO, observes a service failure, or returns the updated profile
...
#### Scenario: Share-token scope rejection diagnostics exclude session values
- **WHEN** the share-token authentication filter rejects a verified token whose scope does not permit a download or save operation
...
#### Scenario: Non-administrator rejection diagnostics exclude identity and path values
- **WHEN** the administrator-authorization filter rejects an authenticated request whose role is not administrator
- **THEN** its directly owned WARN event SHALL use the fixed complete summary `[admin_auth_filter] Non-admin access attempt` with the request's typed correlation and SHALL NOT append the user ID, username, role, status, request path or resource ID, Authorization value, raw access token, JTI, claim, exception, or other administrator-session or request value
- **AND** administrator path classification, authentication-attribute reads, role-before-status authorization order, `AdminRequired` mapping, disabled-administrator rejection, successful continuation, and public responses SHALL remain unchanged

#### Scenario: Disabled-administrator rejection diagnostics exclude identity and path values
- **WHEN** the administrator-authorization filter rejects an authenticated administrator whose status is not active
...
#### Scenario: Share-token issuance diagnostics exclude share values
- **WHEN** share-token generation succeeds
...
#### Scenario: Access and refresh token-pair issuance excludes identity and JTI values
- **WHEN** access and refresh token generation completes successfully
- **THEN** the directly owned DEBUG event SHALL use the fixed complete summary `Token pair generated successfully` with the caller's typed correlation and SHALL NOT append the user ID or name, role, status, access or refresh JTI, JWT secret, raw access or refresh token, claim, signature, issued or expiry time, TTL, exception, or other session-state value
- **AND** independent random JTIs, common issued time, issuer, type, subject, access and refresh claims, access and refresh expiry durations, HS256 signing, and token-pair result SHALL remain unchanged

#### Scenario: Access-token verification diagnostics exclude identity and claim values
- **WHEN** access-token verification succeeds after type and claim parsing
...
#### Scenario: Login diagnostics exclude raw account and limiter values
- **WHEN** login starts, is rate limited, cannot read the limiter, cannot find the account, rejects the password, cannot persist the refresh token, or succeeds
- **THEN** each directly owned event SHALL use its fixed low-cardinality summary with the existing typed correlation and SHALL NOT append the account, client IP, attempt count, user ID, password or hash, token, Redis key or value, downstream error, or other request or response value
- **AND** IP normalization, rate-limit behavior, user and account validation, CPU-pool password verification, failure counting, login-state update, token generation and persistence, response mapping, and public errors SHALL remain unchanged

#### Scenario: Refresh diagnostics exclude token identity values
- **WHEN** refresh starts, verification fails or succeeds, the user is loaded, CAS renewal fails, refresh succeeds, or user query or processing raises an exception
...
#### Scenario: Authentication user lookup diagnostics exclude account values
- **WHEN** authentication user lookup finds no match, succeeds, or encounters a database failure
...
#### Scenario: Login-state update diagnostics exclude user and limiter values
- **WHEN** login-state conditional update loses a race, succeeds, clears or cannot clear its limiter counter, or encounters a database failure
...
#### Scenario: Login request diagnostics exclude raw fields
- **WHEN** login request fields are parsed and pass empty-value validation
...
#### Scenario: Login controller diagnostics exclude boundary values
- **WHEN** the login controller receives a request, rejects DTO validation, receives a business failure, or completes login
- **THEN** each directly owned event SHALL use its fixed low-cardinality summary with the existing typed correlation and SHALL NOT append the client endpoint, username, email-form account, downstream error message, or other request or response value
- **AND** context creation, DTO parsing, trusted client-IP resolution, the service call, error responses, response serialization, and the successful response SHALL remain unchanged

#### Scenario: Refresh controller diagnostics exclude boundary values
- **WHEN** the refresh controller receives a request, rejects DTO validation, receives a business failure, or completes token rotation
- **THEN** each directly owned event SHALL use its fixed low-cardinality summary with the existing typed correlation and SHALL NOT append the client endpoint, refresh token, downstream error message, or other request or response value
- **AND** context creation, DTO and service calls, error responses, token-response serialization, and the successful response SHALL remain unchanged

#### Scenario: Logout controller diagnostics exclude boundary values
- **WHEN** the logout controller receives a request, rejects a missing or malformed header or auth attribute, receives a business failure, or completes logout
- **THEN** each directly owned event SHALL use its fixed low-cardinality summary with the existing typed correlation and SHALL NOT append the client endpoint, Authorization value, access token, user ID, downstream error message, or other request or response value
- **AND** context creation, Bearer extraction, auth-attribute and trusted client-IP reads, the service call, error responses, and the successful response SHALL remain unchanged
...
#### Scenario: Share-list controller diagnostics exclude request and response values
- **WHEN** the share-list controller receives a request, rejects its DTO, completes parameter validation, observes a service failure, or returns a page successfully
- **THEN** its directly owned INFO, WARN, DEBUG, ERROR, and INFO events SHALL use the fixed complete summaries `Received get share list request`, `Share list request parameter validation failed`, `Share list parameter validation passed`, `Get share list failed`, and `Get share list successful` with the request's typed correlation and SHALL NOT append the client IP or port, status, page, page size, DTO or downstream error message, user ID, item count, total count, share ID or code, Authorization value, raw access or share token, or other account, request, or response value
- **AND** DTO parsing and error propagation, authentication-attribute reads, service delegation and correlation, service-error propagation, paginated response mapping, successful response, and public API behavior SHALL remain unchanged

#### Scenario: Share-detail controller diagnostics exclude path and response values
- **WHEN** the share-detail controller receives a request, rejects its path DTO, observes a service failure, or returns share details successfully
- **THEN** its directly owned INFO, WARN, ERROR, and INFO events SHALL use the fixed complete summaries `Received get share details request`, `Share detail request parameter validation failed`, `Get share details failed`, and `Get share details successful` with the request's typed correlation and SHALL NOT append the client IP or port, path or parsed share ID or code, DTO or downstream error message, user ID, file count, Authorization value, raw access or share token, or other account, request, or response value
- **AND** path-DTO parsing and error propagation, authentication-attribute reads, service delegation and correlation, service-error propagation, response mapping, successful response, and public API behavior SHALL remain unchanged

#### Scenario: Share-update controller diagnostics exclude settings and domain values
- **WHEN** the share-update controller receives a request, rejects its DTO, completes parameter validation, observes a service failure, or returns updated settings successfully
- **THEN** its directly owned INFO, WARN, DEBUG, ERROR, and INFO events SHALL use the fixed complete summaries `Received update share settings request`, `Update share settings request parameter validation failed`, `Update share settings parameter validation passed`, `Update share settings failed`, and `Update share settings successful` with the request's typed correlation and SHALL NOT append the client IP or port, path or parsed share ID or code, expiry, password or password-presence state, permission, DTO or downstream error message, user ID, Authorization value, raw access or share token, or other account, request, or response value
- **AND** DTO parsing and error propagation, authentication-attribute reads, service delegation and correlation, service-error propagation, response mapping, successful response, and public API behavior SHALL remain unchanged

#### Scenario: Share-cancel controller diagnostics exclude batch and result values
- **WHEN** the batch share-cancel controller receives a request, rejects its DTO, completes parameter validation, observes a service failure, or returns the batch result successfully
- **THEN** its directly owned INFO, WARN, DEBUG, ERROR, and INFO events SHALL use the fixed complete summaries `Received batch cancel shares request`, `Batch cancel shares request parameter validation failed`, `Batch cancel shares parameter validation passed`, `Batch cancel shares failed`, and `Batch cancel shares completed` with the request's typed correlation and SHALL NOT append the client IP or port, share ID or code or count, DTO or downstream error message, user ID, response total, succeeded, or failed count, Authorization value, raw access or share token, or other account, request, or response value
- **AND** DTO parsing and error propagation, authentication-attribute reads, audit-context construction, service delegation and correlation, service-error propagation, response mapping, partial-success summary semantics, always-HTTP-200 successful response, and public API behavior SHALL remain unchanged

#### Scenario: Share-access controller diagnostics exclude visitor and credential values
- **WHEN** the public share-access controller receives a request, rejects its DTO, completes parameter validation, observes an access rejection, or returns a successful verification response
- **THEN** its directly owned INFO, WARN, DEBUG, WARN, and INFO events SHALL use the fixed complete summaries `Received verify share access request`, `Verify share access request parameter validation failed`, `Verify share access parameter validation passed`, `Verify share access failed`, and `Verify share access successful` with the request's typed correlation and SHALL NOT append the client IP or port, path or parsed share ID or code, password or password-presence state, DTO or downstream error message, permission, generated share token, expiry, file or count, Authorization or `X-Share-Token` value, raw access or share token, or other account, request, or response value
- **AND** the log-only IP alias SHALL be removed while audit-context construction and normalized IP and user-agent data, DTO parsing and error propagation, service delegation and correlation, service-error propagation, response mapping, indistinguishable public failure semantics, password-failure limiting, token response, access audit, successful response, and public API behavior SHALL remain unchanged

#### Scenario: Share-browse controller diagnostics exclude path, token, and result values
- **WHEN** the share-browse controller receives a request, rejects its DTO, completes parameter validation, rejects a token and path share-code mismatch, observes a service failure, or returns browse content successfully
- **THEN** its directly owned INFO, WARN, DEBUG, WARN, ERROR, and INFO events SHALL use the fixed complete summaries `Received browse share content request`, `Browse share content request parameter validation failed`, `Browse share content parameter validation passed`, `Share token does not match requested share`, `Browse share content failed`, and `Browse share content successful` with the request's typed correlation and SHALL NOT append the client IP or port, path or parsed share ID or code, folder ID, DTO or downstream error message, token-attribute share code or internal share ID, item count, Authorization or `X-Share-Token` value, raw access or share token, or other account, request, or response value
- **AND** DTO parsing and error propagation, share-auth attribute reads, token-to-path share-code binding, service delegation and correlation, service-error propagation, response mapping, missing or invalid token and scope rejection, cross-share token rejection, folder navigation, real-time share-state and permission checks, successful response, and public API behavior SHALL remain unchanged

#### Scenario: Share-download-info controller diagnostics exclude path, token, and metadata values
- **WHEN** the share-download-info controller receives a request, rejects its path DTO, rejects a token and path share-code mismatch, observes a service failure, or returns integrity metadata successfully
- **THEN** its directly owned INFO, WARN, WARN, ERROR, and INFO events SHALL use the fixed complete summaries `Received share download info request`, `Share download info request parameter validation failed`, `Share token does not match requested share`, `Get share download info failed`, and `Share download info successful` with the request's typed `download` correlation and SHALL NOT append the client IP or port, path or parsed share ID or code or file ID, DTO or downstream error message, token-attribute share code or internal share ID, filename, size, hash, MIME type, range-support state, Authorization or `X-Share-Token` value, raw access or share token, or other account, request, or response value
- **AND** path-DTO parsing and error propagation, share-auth attribute reads, token-to-path share-code binding, service delegation and correlation, service-error propagation, six-field integrity-metadata response mapping, missing or invalid token and scope rejection, cancelled or expired token rejection, cross-share token rejection, real-time share-state, permission, and file-membership checks, info-request non-counting, successful response, and public API behavior SHALL remain unchanged

#### Scenario: Share-content-download controller diagnostics exclude path, token, and blob values
- **WHEN** the share-content-download controller receives a request, rejects its path DTO, completes parameter validation, rejects a token and path share-code mismatch, observes a metadata-query failure, or obtains the metadata successfully
- **THEN** its directly owned INFO, WARN, DEBUG, WARN, ERROR, and INFO events SHALL use the fixed complete summaries `Received download share file request`, `Download share file request parameter validation failed`, `Download share file parameter validation passed`, `Share token does not match requested share`, `Get download info failed`, and `Get download info successful` with the request's typed `download` correlation and SHALL NOT append the client IP or port, path or parsed share ID or code or file ID, DTO or downstream error message, token-attribute share code or internal share ID, filename, size, content ID or other blob locator, Range, Authorization or `X-Share-Token` value, raw access or share token, or other account, request, or response value
- **AND** path-DTO parsing and error propagation, share-auth attribute reads, token-to-path share-code binding, normalized audit context, metadata service delegation and correlation, metadata-failure response and non-statistical failure audit, blob, filename, size, MIME type, hash, and Range response delegation, actual-response outcome statistics and audit, missing or invalid token and scope rejection, cancelled or expired token rejection, cross-share token rejection, real-time share-state, permission, and file-membership checks, 200, 206, 416, integrity, and storage-failure responses, reconciliation, counters and file-access metadata, and public API behavior SHALL remain unchanged

#### Scenario: Share-save controller diagnostics exclude owner, token, and result values
- **WHEN** the share-save controller receives a request, rejects its DTO, rejects a token and path share-code mismatch, observes a service failure, or returns a successful save result
- **THEN** its directly owned INFO, WARN, WARN, ERROR, and INFO events SHALL use the fixed complete summaries `Received save share items request`, `Save share items request parameter validation failed`, `Share token does not match requested share`, `Save share items failed`, and `Save share items successful` with the request's typed `share` correlation and SHALL NOT append the client IP or port, path or parsed share ID or code, DTO or downstream error message, token-attribute share code or internal share ID, owner or target user ID, file or folder ID or count, target folder, saved, saved-file, or saved-folder count, Authorization or `X-Share-Token` value, raw access or share token, or other account, request, or response value
- **AND** owner JWT authentication and user attribute, real-time share-token verification and download scope, the shared download rate-limit bucket without HTTP-operation reclassification, DTO parsing and error propagation, share-auth attribute reads, token-to-path share-code binding, four-argument service delegation and correlation, service-error propagation, three-field response mapping, missing or invalid owner or share token, cancelled or expired token rejection, cross-share token rejection, real-time share-state, permission, membership, and target-folder ownership checks, recursive copy planning, conflict skips, quota reservation and refund, reference counts, paths and hierarchy, item and parent counts, transaction commit and rollback, post-success cache invalidation, successful response, and public API behavior SHALL remain unchanged
...
#### Scenario: Administrator user-list controller diagnostics exclude request and result values
- **WHEN** the administrator user-list controller receives a request, rejects its DTO, observes a service failure, or returns a successful list
- **THEN** its directly owned INFO, WARN, ERROR, and INFO events SHALL use the fixed complete summaries `Admin list users request`, `List users request validation failed`, `Failed to list users`, and `Admin list users successful` with the request's typed `admin` correlation and SHALL NOT append the client IP or port, raw or parsed page, page size, username, email, status, or role, DTO or downstream error message, administrator or user ID, returned item, pagination or count, Authorization value, raw access token, JWT, JTI, claim, password, storage credential, or other account, request, or response value
- **AND** global JWT authentication, administrator authorization before administration rate limiting, role and status checks, DTO parsing and error propagation, singleton service lookup, request-and-context service delegation, service-error propagation, items-and-pagination response mapping, filtering, count, created-time descending pagination, `int64_t` LIMIT and OFFSET, nullable fields, successful response, and public authentication, authorization, rate-limit, and API behavior SHALL remain unchanged

#### Scenario: Administrator user-detail controller diagnostics exclude request and result values
- **WHEN** the administrator user-detail controller receives a request, observes a service failure, or returns a successful detail
- **THEN** its directly owned INFO, ERROR, and INFO events SHALL use the fixed complete summaries `Admin get user detail request`, `Failed to get user detail`, and `Admin get user detail successful` with the request's typed `admin` correlation and SHALL NOT append the client IP or port, raw or parsed path user ID, downstream error message, returned user ID, username, email, nickname, avatar, role, status, storage quota, storage used, storage reserved, creation time, last-login time, Authorization value, raw access token, JWT, JTI, claim, password, storage credential, or other account, request, or response value
- **AND** global JWT authentication, administrator authorization before administration rate limiting, empty and invalid path-ID validation, numeric conversion, singleton service lookup, user-ID-and-context service delegation, service-error propagation, missing-user mapping, nested `data.user` response mapping, all twelve user-detail fields, successful response, and public authentication, authorization, rate-limit, and API behavior SHALL remain unchanged

#### Scenario: Administrator user-status controller diagnostics exclude request and result values
- **WHEN** the administrator user-status controller receives a request, rejects its DTO, observes a service failure, or returns a successful status change
- **THEN** its directly owned INFO, WARN, ERROR, and INFO events SHALL use the fixed complete summaries `Admin change user status request`, `Change status request validation failed`, `Failed to change user status`, and `Admin change user status successful` with the request's typed `admin` correlation and SHALL NOT append the client IP or port, raw or parsed path user ID, target or operator user ID, status value, DTO or downstream error message, Authorization value, raw access token, JWT, JTI, claim, password, storage credential, or other account, request, or response value
- **AND** global JWT authentication, administrator authorization before administration rate limiting, operator-user-ID attribute lookup, empty and invalid path-ID validation, numeric conversion, required integer status and zero-through-two range validation, DTO-error propagation, singleton service lookup, target-ID, status, operator-ID, and context service delegation, service-error propagation, self-modification rejection, missing-user mapping, status persistence, login-attempt and lock reset, audit recording, successful response, and public authentication, authorization, rate-limit, and API behavior SHALL remain unchanged

#### Scenario: Administrator user-role controller diagnostics exclude request and result values
- **WHEN** the administrator user-role controller receives a request, rejects its DTO, observes a service failure, or returns a successful role change
- **THEN** its directly owned INFO, WARN, ERROR, and INFO events SHALL use the fixed complete summaries `Admin change user role request`, `Change role request validation failed`, `Failed to change user role`, and `Admin change user role successful` with the request's typed `admin` correlation and SHALL NOT append the client IP or port, raw or parsed path user ID, target or operator user ID, role value, DTO or downstream error message, Authorization value, raw access token, JWT, JTI, claim, password, storage credential, or other account, request, or response value
- **AND** global JWT authentication, administrator authorization before administration rate limiting, operator-user-ID attribute lookup, empty and invalid path-ID validation, numeric conversion, required integer role and zero-or-one range validation, DTO-error propagation, singleton service lookup, target-ID, role, operator-ID, and context service delegation, service-error propagation, self-modification rejection, missing-user mapping, last-administrator count and demotion protection, role persistence, audit recording, successful response, and public authentication, authorization, rate-limit, and API behavior SHALL remain unchanged

#### Scenario: Administrator user-available-space controller diagnostics exclude request and result values
- **WHEN** the administrator user-available-space controller receives a request, rejects its DTO, observes a service failure, or returns a successful available-space change
- **THEN** its directly owned INFO, WARN, ERROR, and INFO events SHALL use the fixed complete summaries `Admin change user available space request`, `Change available space request validation failed`, `Failed to change user available space`, and `Admin change user available space successful` with the request's typed `admin` correlation and SHALL NOT append the client IP or port, raw or parsed path user ID, target or operator user ID, available-space GiB value, DTO or downstream error message, returned user ID, username, email, nickname, avatar, role, status, storage quota, storage used, storage reserved, creation time, last-login time, Authorization value, raw access token, JWT, JTI, claim, password, storage credential, or other account, request, or response value
- **AND** global JWT authentication, administrator authorization before administration rate limiting, operator-user-ID attribute lookup, empty and invalid path-ID validation, numeric conversion, required unsigned available-space value and GiB-conversion overflow validation, DTO-error propagation, singleton service lookup, target-ID, available-space, operator-ID, and context service delegation, service-error propagation, self-modification rejection, missing-user mapping, service-side overflow protection, used-plus-reserved-plus-available quota calculation and persistence, audit recording, nested `data.user` mapping with all twelve user-detail fields, successful response, and public authentication, authorization, rate-limit, and API behavior SHALL remain unchanged

#### Scenario: Administrator soft-delete-user controller diagnostics exclude request and result values
- **WHEN** the administrator soft-delete-user controller receives a request, observes a service failure, or returns a successful soft delete
- **THEN** its directly owned INFO, ERROR, and INFO events SHALL use the fixed complete summaries `Admin soft delete user request`, `Failed to soft delete user`, and `Admin soft delete user successful` with the request's typed `admin` correlation and SHALL NOT append the client IP or port, raw or parsed path user ID, target or operator user ID, downstream error message, Authorization value, raw access token, JWT, JTI, claim, password, storage credential, or other account, request, or response value
- **AND** global JWT authentication, administrator authorization before administration rate limiting, operator-user-ID attribute lookup, empty and invalid path-ID validation, numeric conversion, singleton service lookup, target-ID, operator-ID, and context service delegation, service-error propagation, self-deletion rejection, missing-user mapping, database-error propagation, disabled-status persistence, user update, audit recording, successful response, and public authentication, authorization, rate-limit, and API behavior SHALL remain unchanged

#### Scenario: Administrator global-storage-statistics controller diagnostics exclude request and result values
- **WHEN** the administrator global-storage-statistics controller receives a request, observes a service failure, or returns successful statistics
- **THEN** its directly owned INFO, ERROR, and INFO events SHALL use the fixed complete summaries `Admin get global storage stats request`, `Failed to get global storage stats`, and `Admin get global storage stats successful` with the request's typed `admin` correlation and SHALL NOT append the client IP or port, operator user ID, downstream error message, total user count, total file count, total storage used, total storage quota, active share count, audit detail, Authorization value, raw access token, JWT, JTI, claim, password, storage credential, or other account, request, or response value
- **AND** global JWT authentication, administrator authorization before administration rate limiting, operator-user-ID attribute lookup, singleton service lookup, operator-ID-and-context service delegation, service-error propagation, three aggregate queries, empty-result fallback, all five response assignments and JSON fields, real-administrator read-audit recording, successful response, and public authentication, authorization, rate-limit, and API behavior SHALL remain unchanged

#### Scenario: Administrator share-list controller diagnostics exclude query and result values
- **WHEN** the administrator share-list controller receives a request, rejects invalid query parameters, observes a service failure, or returns a successful list
- **THEN** its directly owned INFO, WARN, ERROR, and INFO events SHALL use the fixed complete summaries `Admin list shares request`, `List shares request validation failed`, `Failed to list shares`, and `Admin list shares successful` with the request's typed `admin` correlation and SHALL NOT append the client IP or port, page, page size, status, user ID, username, DTO or downstream error message, operator, share, user, or file ID, list item, pagination or count value, audit detail, Authorization value, raw access token, JWT, JTI, claim, password, storage credential, or other account, request, or response value
- **AND** global JWT authentication, administrator authorization before administration rate limiting, query DTO validation and error propagation, operator-user-ID attribute lookup, singleton service lookup, request-and-operator-ID-and-context service delegation, service-error propagation, empty-share deactivation, filters, count, created-time descending pagination, first-file and nullable-field mapping, real-administrator read-audit recording, item and pagination JSON mapping, successful response, and public authentication, authorization, rate-limit, and API behavior SHALL remain unchanged

#### Scenario: Administrator share-detail controller diagnostics exclude identifiers and result values
- **WHEN** the administrator share-detail controller receives a request, observes a service failure, or returns successful details
- **THEN** its directly owned INFO, ERROR, and INFO events SHALL use the fixed complete summaries `Admin get share detail request`, `Failed to get share detail`, and `Admin get share detail successful` with the request's typed `admin` correlation and SHALL NOT append the client IP or port, raw or parsed path share ID or code, operator, share, user, or file ID, downstream error message, any of the ten response fields, audit detail, Authorization value, raw access token, JWT, JTI, claim, password, storage credential, or other account, request, or response value
- **AND** global JWT authentication, administrator authorization before administration rate limiting, empty-path validation, unchanged external-string share-code propagation without numeric conversion, operator-user-ID attribute lookup, singleton service lookup, share-code-and-operator-ID-and-context service delegation, service-error propagation, empty-share deactivation, share-code query, missing-share mapping, first-file, nullable-field, and PostgreSQL-boolean mapping, real-administrator read-audit recording, direct ten-field JSON mapping, successful response, and public authentication, authorization, rate-limit, and API behavior SHALL remain unchanged

#### Scenario: Administrator force-cancel-share controller diagnostics exclude identifiers and result values
- **WHEN** the administrator force-cancel-share controller receives a request, observes a service failure, or returns a successful cancellation
- **THEN** its directly owned INFO, ERROR, and INFO events SHALL use the fixed complete summaries `Admin force cancel share request`, `Failed to force cancel share`, and `Admin force cancel share successful` with the request's typed `admin` correlation and SHALL NOT append the client IP or port, raw or parsed path share ID or code, operator, share, user, or file ID, downstream error message, status or audit detail, Authorization value, raw access token, JWT, JTI, claim, password, storage credential, or other account, request, or response value
- **AND** global JWT authentication, administrator authorization before administration rate limiting, the DELETE route, operator-user-ID attribute lookup, empty-path validation, unchanged external-string share-code propagation without numeric conversion, singleton service lookup, share-code-and-operator-ID-and-context service delegation, service-error propagation, share-code query, missing-share and already-cancelled mapping, internal-ID status and update-time persistence, four-field real-administrator audit recording, empty successful response, and public authentication, authorization, rate-limit, and API behavior SHALL remain unchanged

#### Scenario: Administrator overview controller diagnostics exclude aggregate and result values
- **WHEN** the administrator overview controller receives a request, observes a service failure, or returns successful statistics
- **THEN** its directly owned INFO, ERROR, and INFO events SHALL use the fixed complete summaries `Admin get overview stats request`, `Failed to get overview stats`, and `Admin get overview stats successful` with the request's typed `admin` correlation and SHALL NOT append the client IP or port, administrator, user, share, or file ID, downstream error message, user, file, or active-share count, storage usage or quota, audit detail, Authorization value, raw access token, JWT, JTI, claim, password, storage credential, or other account, request, or response value
- **AND** global JWT authentication, administrator authorization before administration rate limiting, the GET route, singleton service lookup, context-only service delegation, service-error propagation, user, file, and active-share aggregate queries, empty-result fallback, all five response assignments and JSON fields, successful response, and public authentication, authorization, rate-limit, and API behavior SHALL remain unchanged

#### Scenario: Administrator system-status controller diagnostics exclude dependency and result values
- **WHEN** the administrator system-status controller receives a request, observes a service failure, or returns successful status
- **THEN** its directly owned INFO, ERROR, and INFO events SHALL use the fixed complete summaries `Admin get system status request`, `Failed to get system status`, and `Admin get system status successful` with the request's typed `admin` correlation and SHALL NOT append the client IP or port, administrator, user, share, or file ID, downstream error message, database or Redis connection state, total, used, or free disk space, uptime, dependency exception or diagnostic detail, Authorization value, raw access token, JWT, JTI, claim, password, database, Redis, or storage credential, or other account, request, or response value
- **AND** global JWT authentication, administrator authorization before administration rate limiting, the GET route, singleton service lookup, context-only service delegation, service-error propagation, independent database, Redis, and disk probes, four anonymous exception catches and fixed service warnings, degraded failure assignments, uptime calculation, service success event, all six response assignments and JSON fields, successful response, and public authentication, authorization, rate-limit, and API behavior SHALL remain unchanged

#### Scenario: Administrator operation-log controller diagnostics exclude query and result values
- **WHEN** the administrator operation-log controller receives a request, rejects invalid query parameters, observes a service failure, or returns a successful list
- **THEN** its directly owned INFO, WARN, ERROR, and INFO events SHALL use the fixed complete summaries `Admin list logs request`, `List logs request validation failed`, `Failed to list logs`, and `Admin list logs successful` with the request's typed `admin` correlation and SHALL NOT append the client IP or port, page, page size, action, target type, target name, start date, end date, DTO or downstream error message, administrator, user, share, file, upload, or job ID, any of the nine log-item fields, pagination or count value, audit detail, Authorization value, raw access token, JWT, JTI, claim, password, database, Redis, or storage credential, or other account, request, or response value
- **AND** global JWT authentication, administrator authorization before administration rate limiting, the GET route, seven-query-field DTO validation and error propagation, singleton service lookup, request-and-context service delegation, service-error propagation, five optional exact filters, inclusive date bounds, two parameterized queries, created-time and ID descending order, 64-bit pagination binding, nine log-item fields, four nullable mappings, item and pagination JSON mapping, read-only behavior, successful response, and public authentication, authorization, rate-limit, and API behavior SHALL remain unchanged
...
#### Scenario: Administrator share-list query catches a database exception
- **WHEN** the administrator share-list empty-share update, count, or page query catches a database exception
- **THEN** its directly owned event SHALL use only the fixed complete `Admin list shares database error` summary with typed correlation, and SHALL NOT append administrator, share, user, or file IDs, filter or pagination values, exception text, SQL, connection details, credentials, or tokens
- **AND** PostgreSQL `LIMIT` and `OFFSET` SHALL bind as `int64_t` in both username-filter branches, and PostgreSQL `boolean password_set` SHALL be read with `as<bool>()`, while the existing `InternalError` and `Failed to list shares` public mapping, empty-share update, filters, count, created-time descending pagination, first-file and nullable-field mapping, audit, pagination metadata, and response SHALL remain unchanged
