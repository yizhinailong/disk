...
The system SHALL provide integration safety tests that verify upload lifecycle operations preserve current database and filesystem invariants.
...
- **WHEN** a test prepares or observes an expired in-progress upload and runs the existing cleanup path
...
The system SHALL provide integration safety tests that characterize current content deduplication and reference-count behavior.
...
- **WHEN** trashed files are permanently deleted or expired-trash cleanup runs
...
The system SHALL provide integration safety tests that characterize current quota reservation and storage accounting behavior.
...
- **WHEN** a file is moved to trash and then permanently deleted or removed by expired-trash cleanup
...
The system SHALL run backend safety-net tests through the existing integration-test execution contract.
