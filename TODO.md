...
>
> 最近验证（2026-07-27，本轮 Phase 10 Worker 启动状态死探针清理）：全仓库精确调用点审计确认 `StorageWorkerRuntime::IsStarted()` 只有声明与实现，没有生产、测试或文档调用者；内部 `m_started` 仍由 `Start()` 的原子 CAS 使用，`IsDrained()` 仍由进程关停流程读取。该无调用公开读取器已删除，编译期能力探测禁止其回归；幂等启动、定时触发、drain admission、in-flight 排空观察和日志语义不变。完整构建、Worker 运行时/进程关停/真实排空接管/分布式拓扑聚焦 CTest 17/17、OpenSpec 24/24 通过。完整 CTest 共 1456 项，1449 通过、7 项按环境门控跳过，0 失败，总耗时 487.81 秒；环境门控用例仍须在目标 MinIO/云 S3 和多实例拓扑中执行。
