...
>
> 最近验证（2026-07-28，本轮 Phase 10 文件仓储死客户端状态清理）：全仓库精确读写与实例化审计确认 `FileRepository` 构造参数只写入 `m_db_client`，该字段从未被任何方法读取；全部 6 个生产仓储方法已经显式接收调用级 standalone client 或当前 transaction。无读取字段、注入构造及四个旧构造点已删除，仓储现在可默认构造且为空类型；编译期/源码合同禁止伪依赖回归并锁定 6 个显式 client 参数，文件重命名、移动、文件夹重命名/删除计划继续传入原连接。完整构建、无状态文件仓储/文件夹仓储/移动合同、真实文件夹生命周期/嵌套移动安全网、分布式拓扑聚焦 CTest 15/15、OpenSpec 24/24 通过。完整 CTest 共 1457 项，1450 通过、7 项按环境门控跳过，0 失败，总耗时 504.68 秒；环境门控用例仍须在目标 MinIO/云 S3 和多实例拓扑中执行。
