...
### 15.195 UserService 目标账户诊断脱敏记录（2026-08-04）

`GetProfile()` 读取到用户行与 `ChangePassword()` 加载到目标账户时，两条 DEBUG 事件必须分别只记录固定完整摘要 `User profile record loaded` 与 `Password change target loaded`；message 不得追加用户名、邮箱、昵称、头像、用户 ID、密码/哈希或其他账户与请求值。两条事件继续使用调用方 request/instance/`user` 类型化上下文，并保持原级别和数据加载后的触发位置。

资料聚合查询及全部响应字段映射、旧密码验证、与当前密码相同的拒绝、密码哈希、密码字段赋值、单次用户更新、结构化未找到与依赖错误映射、Controller/DTO、鉴权和公开响应不得改变。验证必须新增源码合同锁定两条固定摘要、原始账户资料排除、资料响应映射和完整密码变更链，并执行 UserService/密码 DTO GoogleTest、OpenSpec、完整构建、真实资料读取/密码修改流及标准完整 CTest。Phase 10 与最终 Definition of Done 在其余迁移、兼容退役和目标环境门禁完成前继续保持未勾选。

旧实现上的定向源码合同按预期为 0/1，共 4 个失败断言：资料读取与密码修改的固定摘要均缺失，两条目标加载事件仍各拼接 1 次用户名；资料 11 项响应映射和旧密码验证/同密码拒绝/哈希/密码赋值/单次更新 5 个关键步骤均通过。
...
### 15.199 注册控制器诊断脱敏记录（2026-08-04）

`AuthController::Register()` 的请求接收、DTO 校验失败、参数校验完成、业务失败和注册成功五条事件，必须分别只记录固定完整摘要 `Received user registration request`、`User registration request validation failed`、`User registration parameters validated`、`User registration business logic failed` 与 `User registration successful`。message 不得追加客户端 IP/端口、用户名、邮箱、用户 ID、下游错误消息或其他注册请求/响应值；原 INFO/WARN/DEBUG/ERROR/INFO 级别、调用方 request/instance/`auth` 上下文和分支触发位置保持不变。

既有请求上下文创建、`RegisterRequest::FromRequest()` 调用、DTO/业务错误响应、`AuthService::Register()` 调用、用户 JSON 包装和成功响应不得改变。验证必须新增源码合同锁定五条固定摘要、原始边界值排除和完整控制器数据流，并执行认证日志/DTO GoogleTest、OpenSpec、完整构建、真实认证流/生命周期、上传安全网及标准完整 CTest。Phase 10 与最终 Definition of Done 在其余迁移、兼容退役和目标环境门禁完成前继续保持未勾选。

旧实现上的定向源码合同按预期为 0/1，共 10 个失败断言：五条固定摘要均缺失，请求端点、DTO 错误/用户名、业务错误/用户名及成功用户名/用户 ID 五条动态日志各命中一次；上下文、DTO/Service 调用、两类错误响应、JSON 包装和成功响应断言均通过。

实现后定向源码合同 1/1、含 Auth 的 GoogleTest 115/115、`RegisterRequest` DTO 24/24、OpenSpec 24/24 和完整后端构建通过；真实认证流 8/8、认证生命周期 6/6、上传安全网 886/886 均通过、0 失败。标准完整 CTest 共 1530 项：1523 项通过、PgBouncer/Prometheus、3 项 S3 与 2 项分布式目标环境门控共 7 项跳过、0 失败，`LastTest.log` 累计测试用时 550.88 秒。源码审计确认五条固定摘要各 1 处、五种旧动态边界日志均为 0，完整注册控制器数据流继续由源码合同锁定。Phase 10 与最终 Definition of Done 在其余迁移、兼容退役和目标环境门禁完成前继续保持未勾选。

### 15.200 登录控制器诊断脱敏记录（2026-08-05）

`AuthController::Login()` 的请求接收、DTO 校验失败、业务失败和登录成功四条事件，必须分别只记录固定完整摘要 `Received login request`、`Login request validation failed`、`Login failed` 与 `Login successful`。message 不得追加客户端 IP/端口、用户名、邮箱形式账号、下游错误消息或其他登录请求/响应值；原 INFO/WARN/ERROR/INFO 级别、调用方 request/instance/`auth` 上下文和分支触发位置保持不变。

既有请求上下文创建、`LoginRequest::FromRequest()` 调用、可信客户端 IP 解析、`AuthService::Login()` 调用、DTO/业务错误响应、令牌与用户响应序列化和成功响应不得改变。验证必须新增源码合同锁定四条固定摘要、原始边界值排除和完整控制器数据流，并执行认证日志/DTO GoogleTest、OpenSpec、完整构建、真实认证流/生命周期、上传安全网及标准完整 CTest。Phase 10 与最终 Definition of Done 在其余迁移、兼容退役和目标环境门禁完成前继续保持未勾选。

旧实现上的定向源码合同按预期为 0/1，共 8 个失败断言：四条固定摘要均缺失，请求端点、DTO 错误、业务错误和成功账号四条动态日志各命中一次；上下文、DTO/IP 解析、Service 调用、两类错误响应和成功序列化响应断言均通过。
...
### 15.202 登出控制器诊断脱敏记录（2026-08-05）

`AuthController::Logout()` 的请求接收、Authorization 缺失、Authorization 格式错误、认证属性缺失、业务失败和登出成功六条事件，必须分别只记录固定完整摘要 `Received logout request`、`Logout request missing Authorization header`、`Logout request Authorization header format invalid`、`Logout request missing user_id attribute`、`Logout failed` 与 `Logout successful`。message 不得追加客户端 IP/端口、Authorization 值、access token、用户 ID、下游错误消息或其他登出请求/响应值；原 INFO/WARN/WARN/WARN/ERROR/INFO 级别、调用方 request/instance/`auth` 上下文和分支触发位置保持不变，既有三条拒绝摘要继续保持原样。

既有请求上下文创建、Authorization 读取与 Bearer 前缀校验/提取、认证用户属性校验/读取、可信客户端 IP 解析、`AuthService::Logout()` 调用、四类错误响应和成功响应不得改变。验证必须新增源码合同锁定六条固定摘要、原始边界值排除和完整控制器数据流，并执行认证日志 GoogleTest、OpenSpec、完整构建、真实登出/认证生命周期、上传安全网及标准完整 CTest。Phase 10 与最终 Definition of Done 在其余迁移、兼容退役和目标环境门禁完成前继续保持未勾选。

旧实现上的定向源码合同按预期为 0/1，共 6 个失败断言：请求接收、业务失败和成功三条固定摘要均缺失，对应端点、下游错误和用户 ID 动态日志各命中一次；既有三条拒绝固定摘要、2/3/1 级别分布、上下文、Bearer/用户属性/IP 读取、Service 调用和四类错误/成功响应断言均通过。
...
### 15.212 分享令牌验签成功诊断脱敏记录（2026-08-05）

`TokenService::VerifyShareToken()` 的验签成功事件必须只记录固定完整摘要 `Share token verification successful`。message 不得追加分享码、内部分享 ID、权限、JTI、JWT secret、原始分享 token、claim/scope、验签/解析结果、异常或其他分享会话状态值；原 DEBUG 级别、调用方 request/instance/`share` 上下文和全部校验通过后的触发位置保持不变。

既有空 token 拒绝、JWT 解码、已初始化或临时密钥 verifier 选择、issuer/签名/过期验证、type/share code/JTI/subject/scope 提取与完整性校验、`view`/`download` 权限约束、结构化 claims 返回、固定失败摘要和错误映射不得改变。验证必须新增源码合同锁定固定成功摘要、原始分享值排除和完整验签数据流，并执行 Token/share GoogleTest、OpenSpec、完整构建、真实分享令牌安全流、双实例认证一致性、上传安全网及标准完整 CTest。Phase 10 与最终 Definition of Done 在其余迁移、兼容退役和目标环境门禁完成前继续保持未勾选。

旧实现上的定向源码合同按预期为 0/1，共 3 个失败断言：待迁移固定成功摘要缺失，分享码与内部分享 ID 动态日志片段各命中一次；空 token 拒绝、verifier 选择、type/claims/scope/权限校验、结构化结果、固定失败摘要和错误映射共 35 项验签数据流断言均通过。
...
### 15.217 JWT 撤销检查诊断脱敏记录（2026-08-05）

`JwtAuthFilter::doFilter()` 的 access token 撤销依赖失败与已撤销拒绝事件必须分别只记录固定完整摘要 `Access token revocation check failed` 与 `Access token has been revoked`。两条 message 不得追加用户 ID、用户名、JTI、Authorization、原始 access token、Redis key/value、撤销结果/错误、缓存状态、TTL、异常或其他认证会话状态值；调用方 request/instance/operation 上下文、原 ERROR/WARN 级别和原条件触发位置保持不变。

既有单次 access token 验签、结构化 claims、携带 JTI 与调用方上下文的共享 Redis 撤销查询、依赖失败 fail closed 与原错误码透传、已撤销 `TokenRevoked` 映射、认证属性注入、耗时事件和公开响应不得改变。验证必须新增源码合同锁定固定摘要、身份/JTI 值排除和两条分支的数据流，并执行 auth filter/Token GoogleTest、OpenSpec、完整构建、真实认证与 refresh 流、双实例认证一致性、上传安全网及标准完整 CTest。Phase 10 与最终 Definition of Done 在其余认证诊断迁移、兼容退役和目标环境门禁完成前继续保持未勾选。

旧实现上的定向源码合同按预期为 0/1，共 5 个失败断言：两条固定摘要均缺失，依赖失败分支的用户 ID 与已撤销分支的用户 ID/JTI 三处原始 claims 各命中一次；撤销查询、错误透传、`TokenRevoked`、耗时失败事件和四个认证属性断言均通过。
...
### 15.220 分享令牌 Scope 拒绝诊断脱敏记录（2026-08-05）

`ShareAuthFilter::doFilter()` 的操作 scope 拒绝事件必须只记录固定完整摘要 `Share token scope does not permit operation`。message 不得追加分享码、内部分享 ID、permission、请求路径或其中的文件段、JTI、`X-Share-Token`、原始 Share Token、Redis 状态、异常或其他分享会话/请求值；调用方 request/instance/operation 上下文、原 WARN 级别和 scope 判定后的触发位置保持不变。

既有 `RequiresDownloadScope(request->path())`、download/save 前缀判定、`permission != "download"` 条件、`ShareAccessDenied` 映射、拒绝时不注入 `share_code`/`share_id`/`share_token_jti`、成功属性注入与耗时事件不得改变。验证必须新增源码合同锁定固定摘要、三类动态值排除和拒绝分支数据流，并继续执行 ShareAuthFilter/Token GoogleTest、分享 token 安全与限流真实流、双实例认证一致性、上传安全网、OpenSpec、完整构建及标准完整 CTest。成功耗时事件继续作为独立边界审计；Phase 10 与最终 Definition of Done 在其余认证诊断迁移、兼容退役和目标环境门禁完成前保持未勾选。

旧实现上的定向源码合同按预期为 0/1，共 4 个失败断言：固定完整摘要缺失，WARN 中的分享码、permission 与请求路径三类动态值各命中一次；scope 条件、`ShareAccessDenied`、三项属性位置和成功耗时事件断言均通过。
...
### 15.223 禁用管理员访问拒绝诊断脱敏记录（2026-08-05）

`AdminAuthFilter::doFilter()` 的禁用或非 active 管理员拒绝事件必须只记录固定完整摘要 `[admin_auth_filter] Disabled admin access`。message 不得追加用户 ID、用户名、role、status、请求路径或其中的资源 ID、Authorization、原始 access token、JTI、claim、异常或其他管理员会话/请求值；调用方 request/instance/`admin` 上下文、原 WARN 级别和 status 判定后的触发位置保持不变。

既有 `/api/admin/` 前缀门控、`user_id`/`role`/`status` 三项认证属性读取、固定非管理员摘要与 role 优先判定、`status != 1` 条件、`AdminRequired` 映射和管理员成功放行分支不得改变。验证必须新增源码合同锁定固定摘要、三类动态值排除和状态拒绝完整数据流，并继续执行 AdminAuthFilter/auth filter GoogleTest、真实认证流、双实例认证一致性、上传安全网、OpenSpec、完整构建及标准完整 CTest。管理员成功放行事件继续作为独立边界审计；Phase 10 与最终 Definition of Done 在其余认证诊断迁移、兼容退役和目标环境门禁完成前保持未勾选。

旧实现上的定向源码合同按预期为 0/1，共 4 个失败断言：固定完整摘要缺失，status 分支日志中的用户 ID、status 与请求路径三类动态值各命中一次；固定非管理员摘要、role/status 顺序、管理员路径门控、三项属性读取、`AdminRequired` 和两个放行位置断言均通过。

实现后定向源码合同 1/1、role/status/成功行为 4/4、AdminAuthFilter 聚焦 CTest 37/37、全部 AuthFilter GoogleTest 104/104、OpenSpec 24/24 和完整后端构建通过；真实认证流 8/8、双 API 认证一致性 1/1、上传安全网直接执行 886/886 均通过、0 失败。首轮标准完整 CTest 唯一失败为未改动的内容清理收敛检查仍短暂看到 content 行，定向复跑 1/1 通过；第二轮标准完整 CTest 共 1554 项：1547 项通过、PgBouncer/Prometheus、3 项 S3 与 2 项分布式目标环境门控共 7 项跳过、0 失败，总耗时 561.45 秒。源码审计确认固定禁用管理员拒绝摘要 1 处，目标 WARN 中用户 ID、status 与请求路径三类动态片段均为 0；管理员路径门控、三项属性读取、固定 role 摘要、判定顺序、`AdminRequired`、两个放行位置和 WARN 级别保持不变。管理员成功放行事件继续作为独立边界审计；Phase 10 与最终 Definition of Done 在其余认证诊断迁移、兼容退役和目标环境门禁完成前继续保持未勾选。

### 15.224 管理员授权成功诊断脱敏记录（2026-08-05）

`AdminAuthFilter::doFilter()` 的管理员授权成功事件必须只记录固定完整摘要 `[admin_auth_filter] Admin access granted`。message 不得追加用户 ID、用户名、role、status、请求路径或其中的资源 ID、Authorization、原始 access token、JTI、claim、异常或其他管理员会话/请求值；调用方 request/instance/`admin` 上下文、原 TRACE 级别和成功放行前的触发位置保持不变。

既有 `/api/admin/` 前缀门控、`user_id`/`role`/`status` 三项认证属性读取、固定 role/status 拒绝摘要、role 优先判定、`status != 1` 条件、两条 `AdminRequired` 映射及最终 `co_return nullptr` 不得改变。验证必须新增源码合同锁定固定摘要、用户 ID/path 动态值排除和成功分支完整数据流，并继续执行 AdminAuthFilter/auth filter GoogleTest、真实认证流、双实例认证一致性、上传安全网、OpenSpec、完整构建及标准完整 CTest。Phase 10 与最终 Definition of Done 在其余认证诊断迁移、兼容退役和目标环境门禁完成前保持未勾选。

旧实现上的定向源码合同按预期为 0/1，共 3 个失败断言：固定完整摘要缺失，成功 TRACE 中的用户 ID 与请求路径两类动态值各命中一次；管理员路径门控、三项属性读取、role/status 顺序、两条固定拒绝摘要、两条 `AdminRequired` 和两个放行位置断言均通过。

实现后三条管理员日志源码合同 3/3、active/disabled/locked 行为 3/3、AdminAuthFilter 聚焦 CTest 38/38、全部 AuthFilter GoogleTest 105/105、OpenSpec 24/24 和完整后端构建通过；真实认证流 8/8、双 API 认证一致性 1/1、上传安全网直接执行 886/886 均通过、0 失败。标准完整 CTest 共 1555 项：1548 项通过、PgBouncer/Prometheus、3 项 S3 与 2 项分布式目标环境门控共 7 项跳过、0 失败，总耗时 565.74 秒。源码审计确认固定管理员授权成功摘要 1 处，目标 TRACE 中用户 ID 与请求路径两类动态片段均为 0；管理员路径门控、三项属性读取、固定 role/status 拒绝摘要、判定顺序、两条 `AdminRequired`、两个放行位置和 TRACE 级别保持不变。Phase 10 与最终 Definition of Done 在其余认证诊断迁移、兼容退役和目标环境门禁完成前继续保持未勾选。

### 15.225 用户资料读取控制器诊断脱敏记录（2026-08-05）

`UserController::GetProfile()` 的请求接收、业务失败和读取成功三条事件必须分别只记录固定完整摘要 `Received user info request`、`Failed to get user info` 与 `User info retrieved successfully`。message 不得追加客户端 IP/端口、用户 ID、用户名、邮箱、昵称、头像、下游错误消息、Authorization、原始 access token、密码/哈希或其他用户请求/响应值；调用方 request/instance/`user` 上下文、原 INFO/ERROR/INFO 级别和分支触发位置保持不变。

既有上下文创建、`user_id` 认证属性读取、`UserService::GetProfile()` 调用及上下文传播、下游错误原样返回、`data["user"]` 响应包装和成功响应不得改变。验证必须新增源码合同锁定三条固定摘要、三类动态边界值排除和完整控制器数据流，并继续执行 UserLogContext/UserService GoogleTest、真实资料读取与存储流、上传安全网、OpenSpec、完整构建及标准完整 CTest。密码、资料更新和存储控制器继续作为独立边界审计；Phase 10 与最终 Definition of Done 在其余诊断迁移、兼容退役和目标环境门禁完成前保持未勾选。

旧实现上的定向源码合同按预期为 0/1，共 6 个失败断言：三条固定完整摘要均缺失，请求端点、下游错误消息和成功用户 ID 三类动态日志各命中一次；上下文、认证属性、Service 调用、错误透传、用户 JSON 包装、成功响应和 INFO/ERROR/INFO 级别分布断言均通过。

实现后定向源码合同 1/1、三条 User 值日志合同 3/3、用户聚焦 CTest 10/10、名称含 User 的 GoogleTest 88/88、OpenSpec 24/24 和完整后端构建通过；真实资料读取与存储流 5/5、上传安全网直接执行 886/886 均通过、0 失败。标准完整 CTest 共 1556 项：1549 项通过、PgBouncer/Prometheus、3 项 S3 与 2 项分布式目标环境门控共 7 项跳过、0 失败，总耗时 568.71 秒。源码审计确认三条固定控制器摘要各 1 处，请求端点、下游错误消息和成功用户 ID 三类旧动态日志均为 0；上下文、认证属性、Service 调用、错误透传、用户 JSON 包装、成功响应和 INFO/ERROR/INFO 级别分布保持不变。密码、资料更新和存储控制器继续作为独立边界审计；Phase 10 与最终 Definition of Done 在其余诊断迁移、兼容退役和目标环境门禁完成前继续保持未勾选。

### 15.226 密码修改控制器诊断脱敏记录（2026-08-05）

`UserController::UpdatePassword()` 的请求接收、DTO 校验失败、业务失败和修改成功四条事件必须分别只记录固定完整摘要 `Received change password request`、`Change password request validation failed`、`Failed to change password` 与 `Change password successful`。message 不得追加客户端 IP/端口、用户 ID、用户名、邮箱、原密码、新密码、密码哈希、DTO/下游错误消息、Authorization、原始 access token 或其他账户/请求/响应值；调用方 request/instance/`user` 上下文、原 INFO/WARN/ERROR/INFO 级别和分支触发位置保持不变。

既有上下文创建、`user_id` 认证属性读取、`ChangePasswordRequest::FromRequest()` 解析及上下文传播、DTO 错误原样返回、`UserService::ChangePassword()` 调用及上下文传播、业务错误原样返回和空 data 成功响应不得改变。验证必须新增源码合同锁定四条固定摘要、四类动态边界值排除和完整控制器数据流，并继续执行 UserLogContext/UserDto/UserService GoogleTest、真实密码修改流、上传安全网、OpenSpec、完整构建及标准完整 CTest。资料更新和存储控制器继续作为独立边界审计；Phase 10 与最终 Definition of Done 在其余诊断迁移、兼容退役和目标环境门禁完成前保持未勾选。

旧实现上的定向源码合同按预期为 0/1，共 8 个失败断言：四条固定完整摘要均缺失，请求端点、DTO 错误消息、下游错误消息和成功用户 ID 四类动态日志各命中一次；上下文、认证属性、DTO/Service 调用、两类错误透传、空 data 成功响应和 INFO/WARN/ERROR/INFO 级别分布断言均通过。

实现后定向源码合同 1/1、四条 User 值日志合同 4/4、用户聚焦 CTest 10/10、`ChangePasswordRequest` DTO 15/15、名称含 User 的 GoogleTest 89/89、OpenSpec 24/24 和完整后端构建通过；真实密码修改流 6/6、上传安全网直接执行 886/886 均通过、0 失败，原密码已恢复。标准完整 CTest 共 1557 项：1550 项通过、PgBouncer/Prometheus、3 项 S3 与 2 项分布式目标环境门控共 7 项跳过、0 失败，总耗时 570.12 秒。源码审计确认四条固定控制器摘要各 1 处，请求端点、DTO 错误消息、下游错误消息和成功用户 ID 四类旧动态日志均为 0；上下文、认证属性、DTO/Service 调用、两类错误透传、空 data 成功响应和 INFO/WARN/ERROR/INFO 级别分布保持不变。资料更新和存储控制器继续作为独立边界审计；Phase 10 与最终 Definition of Done 在其余诊断迁移、兼容退役和目标环境门禁完成前继续保持未勾选。

### 15.227 用户资料更新控制器诊断脱敏记录（2026-08-06）

`UserController::UpdateProfile()` 的请求接收、DTO 校验失败、业务失败和更新成功四条事件必须分别只记录固定完整摘要 `Received profile update request`、`Profile update request validation failed`、`Failed to update profile` 与 `Profile update successful`。message 不得追加客户端 IP/端口、用户 ID、用户名、邮箱、昵称、头像、DTO/下游错误消息、Authorization、原始 access token、密码/哈希或其他账户/请求/响应值；调用方 request/instance/`user` 上下文、原 INFO/WARN/ERROR/INFO 级别和分支触发位置保持不变。

既有上下文创建、`user_id` 认证属性读取、`UpdateProfileRequest::FromRequest()` 解析及上下文传播、DTO 错误原样返回、`UserService::UpdateProfile()` 调用及上下文传播、业务错误原样返回、`data["user"]` 响应包装和成功响应不得改变。验证必须新增源码合同锁定四条固定摘要、四类动态边界值排除和完整控制器数据流，并继续执行 UserLogContext/UserDto/UserService GoogleTest、真实资料更新流、上传安全网、OpenSpec、完整构建及标准完整 CTest。存储控制器继续作为独立边界审计；Phase 10 与最终 Definition of Done 在其余诊断迁移、兼容退役和目标环境门禁完成前保持未勾选。

旧实现上的定向源码合同按预期为 0/1，共 8 个失败断言：四条固定完整摘要均缺失，请求端点、DTO 错误消息、下游错误消息和成功用户 ID 四类动态日志各命中一次；上下文、认证属性、DTO/Service 调用、两类错误透传、用户 JSON 包装、成功响应和 INFO/WARN/ERROR/INFO 级别分布断言均通过。
...
### 15.230 分享列表控制器诊断脱敏记录（2026-08-06）

`ShareController::List()` 的请求接收、DTO 校验失败、参数校验完成、业务失败和查询成功五条事件必须分别只记录固定完整摘要 `Received get share list request`、`Share list request parameter validation failed`、`Share list parameter validation passed`、`Get share list failed` 与 `Get share list successful`。message 不得追加客户端 IP/端口、status/page/page_size、DTO/下游错误消息、用户 ID、列表项数量、总数、分享 ID/码、Authorization、原始 access/share token 或其他账户/请求/响应值；调用方 request/instance/`share` 上下文、原 INFO/WARN/DEBUG/ERROR/INFO 级别和分支触发位置保持不变。

既有上下文创建、`ShareListRequest::FromRequest()` 解析及错误透传、认证用户属性读取、`ShareService::List()` 调用及上下文传播、业务错误原样返回、`ShareListResponse::ToJson()` 分页映射和成功响应不得改变。验证必须新增源码合同锁定五条固定摘要、五类动态边界值排除和完整控制器数据流，并继续执行 ShareLogContext/ShareDto/ShareService GoogleTest、真实分享浏览与列表流、上传安全网、OpenSpec、完整构建及标准完整 CTest。其余八个分享控制器入口继续独立审计；Phase 10 与最终 Definition of Done 在其余诊断迁移、兼容退役和目标环境门禁完成前保持未勾选。

旧实现上的定向源码合同按预期为 0/1，共 10 个失败断言：五条固定完整摘要均缺失，请求端点、DTO 错误消息、分页筛选参数、下游错误/用户 ID 和成功项数/总数/用户 ID 五类动态日志各命中一次；上下文、DTO 解析、认证属性、Service 调用、两类错误透传、分页响应映射、成功响应和 INFO/WARN/DEBUG/ERROR/INFO 级别分布断言均通过。

实现后两个分享控制器值日志合同 2/2、分享聚焦 CTest 53/53、名称含 Share 的 GoogleTest 292/292、OpenSpec 24/24 和完整后端构建通过；真实分享浏览与列表 9/9、上传安全网直接执行 886/886 均通过、0 失败。标准完整 CTest 共 1561 项：1554 项通过、PgBouncer/Prometheus、3 项 S3 与 2 项分布式目标环境门控共 7 项跳过、0 失败，总耗时 540.37 秒。源码审计确认五条列表控制器固定摘要各 1 处，请求端点、DTO 错误消息、分页筛选参数、下游错误/用户 ID 和成功项数/总数/用户 ID 五类旧动态日志均为 0；上下文、DTO 解析、认证属性、Service 调用、两类错误透传、分页响应映射、成功响应和 INFO/WARN/DEBUG/ERROR/INFO 级别分布保持不变。其余八个分享控制器入口继续作为独立边界审计；Phase 10 与最终 Definition of Done 在其余诊断迁移、兼容退役和目标环境门禁完成前继续保持未勾选。

### 15.231 分享详情控制器诊断脱敏记录（2026-08-06）

`ShareController::Detail()` 的请求接收、路径参数校验失败、业务失败和查询成功四条事件必须分别只记录固定完整摘要 `Received get share details request`、`Share detail request parameter validation failed`、`Get share details failed` 与 `Get share details successful`。message 不得追加客户端 IP/端口、路径或解析后的分享 ID/码、DTO/下游错误消息、用户 ID、文件数量、Authorization、原始 access/share token 或其他账户/请求/响应值；调用方 request/instance/`share` 上下文、原 INFO/WARN/ERROR/INFO 级别和分支触发位置保持不变。

既有上下文创建、`ShareDetailRequest::FromPath()` 解析及错误透传、认证用户属性读取、`ShareService::Detail()` 调用及上下文传播、业务错误原样返回、`ShareDetailResponse::ToJson()` 映射和成功响应不得改变。验证必须新增源码合同锁定四条固定摘要、四类动态边界值排除和完整控制器数据流，并继续执行 ShareLogContext/ShareDto/ShareService GoogleTest、真实分享管理流、上传安全网、OpenSpec、完整构建及标准完整 CTest。其余七个分享控制器入口继续独立审计；Phase 10 与最终 Definition of Done 在其余诊断迁移、兼容退役和目标环境门禁完成前保持未勾选。

旧实现上的定向源码合同按预期为 0/1，共 8 个失败断言：四条固定完整摘要均缺失，请求端点/分享 ID、DTO 错误消息、下游错误/用户与分享 ID 和成功分享 ID/文件数/用户 ID 四类动态日志各命中一次；上下文、路径 DTO 解析、认证属性、Service 调用、两类错误透传、响应映射、成功响应和 INFO/WARN/ERROR/INFO 级别分布断言均通过。
...
### 15.233 分享批量取消控制器诊断脱敏记录（2026-08-06）

`ShareController::Cancel()` 的请求接收、DTO 校验失败、参数校验完成、业务失败和批量完成五条事件必须分别只记录固定完整摘要 `Received batch cancel shares request`、`Batch cancel shares request parameter validation failed`、`Batch cancel shares parameter validation passed`、`Batch cancel shares failed` 与 `Batch cancel shares completed`。message 不得追加客户端 IP/端口、分享 ID/码或数量、DTO/下游错误消息、用户 ID、响应 total/succeeded/failed 计数、Authorization、原始 access/share token 或其他账户/请求/响应值；调用方 request/instance/`share` 上下文、原 INFO/WARN/DEBUG/ERROR/INFO 级别和分支触发位置保持不变。

既有上下文创建、`CancelShareRequest::FromRequest()` 解析及错误透传、认证用户属性读取、`BuildAuditContext()`、`ShareService::Cancel()` 调用及上下文传播、业务错误原样返回、`CancelShareResponse::ToJson()` 映射、批量部分成功汇总和始终 HTTP 200 的成功响应不得改变。验证必须新增源码合同锁定五条固定摘要、五类动态边界值排除和完整控制器数据流，并继续执行 ShareLogContext/ShareDto/ShareService 批量 GoogleTest、真实分享管理流、上传安全网、OpenSpec、完整构建及标准完整 CTest。其余五个分享控制器入口继续独立审计；Phase 10 与最终 Definition of Done 在其余诊断迁移、兼容退役和目标环境门禁完成前保持未勾选。

旧实现上的定向源码合同按预期为 0/1，共 10 个失败断言：五条固定完整摘要均缺失，请求端点、DTO 错误消息、分享 ID 数量、下游错误/用户 ID 和响应 total/succeeded/failed 计数/用户 ID 五类动态日志各命中一次；上下文、DTO 解析、认证属性、审计上下文、Service 调用、两类错误透传、响应映射、成功响应和 INFO/WARN/DEBUG/ERROR/INFO 级别分布断言均通过。

实现后定向源码合同 1/1、五个分享控制器值日志合同 5/5、分享聚焦 CTest 56/56、Cancel DTO/批量语义 GoogleTest 13/13、名称含 Share 的 GoogleTest 295/295、OpenSpec 24/24 和完整后端构建通过；真实分享管理 12/12、上传安全网直接执行 888/888 均通过、0 失败。标准完整 CTest 共 1564 项：1557 项通过、PgBouncer/Prometheus、3 项 S3 与 2 项分布式目标环境门控共 7 项跳过、0 失败，总耗时 541.00 秒。源码审计确认五条批量取消控制器固定摘要各 1 处，请求端点、DTO 错误消息、分享 ID 数量、下游错误/用户 ID 和响应 total/succeeded/failed 计数/用户 ID 五类旧动态日志均为 0；上下文、DTO 解析、认证属性、审计上下文、Service 调用、两类错误透传、响应映射、批量部分成功与始终 HTTP 200 的成功响应和 INFO/WARN/DEBUG/ERROR/INFO 级别分布保持不变。其余五个分享控制器入口继续作为独立边界审计；Phase 10 与最终 Definition of Done 在其余诊断迁移、兼容退役和目标环境门禁完成前继续保持未勾选。

### 15.234 分享访问验证控制器诊断脱敏记录（2026-08-06）

`ShareController::Access()` 的请求接收、DTO 校验失败、参数校验完成、业务拒绝和验证成功五条事件必须分别只记录固定完整摘要 `Received verify share access request`、`Verify share access request parameter validation failed`、`Verify share access parameter validation passed`、`Verify share access failed` 与 `Verify share access successful`。message 不得追加客户端 IP/端口、路径或解析后的分享 ID/码、密码或密码存在状态、DTO/下游错误消息、权限、生成的 Share Token、有效期、文件或数量、Authorization、`X-Share-Token`、原始 access/share token 或其他账户/请求/响应值；调用方 request/instance/`share` 上下文、原 INFO/WARN/DEBUG/WARN/INFO 级别和分支触发位置保持不变。只服务于旧动态日志的 `ip_address` 局部别名必须删除，`BuildAuditContext()` 及其归一化 IP/UA 数据不得删除或改写。

既有上下文创建、`BuildAuditContext()` 构造、`AccessShareRequest::FromRequest()` 解析及错误透传、`ShareService::Access()` 调用及审计/日志上下文传播、业务错误原样返回、`AccessShareResponse::ToJson()` 映射和成功响应不得改变。公开访问对不存在分享、缺失/空/错误密码的不可区分失败，密码失败计数与限流，成功 Share Token/权限/文件响应和访问审计行为保持不变。验证必须新增源码合同锁定五条固定摘要、五类动态边界值排除、日志专用别名删除和完整控制器数据流，并继续执行 ShareLogContext/ShareDto/ShareService GoogleTest、真实分享管理/密码保护/审计流、上传安全网、OpenSpec、完整构建及标准完整 CTest。其余四个分享控制器入口继续独立审计；Phase 10 与最终 Definition of Done 在其余诊断迁移、兼容退役和目标环境门禁完成前保持未勾选。

旧实现上的定向源码合同按预期为 0/1，共 11 个失败断言：五条固定完整摘要均缺失，请求端点/分享码、DTO 错误消息、解析后的分享码/密码存在状态、下游错误/分享码/IP 和成功分享码/权限/IP 五类动态日志各命中一次，只服务于日志的 `ip_address` 别名命中一次；上下文、审计上下文、DTO 解析、Service 调用、两类错误透传、响应映射、成功响应和 INFO/WARN/DEBUG/WARN/INFO 级别分布断言均通过。
...
### 15.236 分享下载信息控制器诊断脱敏记录（2026-08-06）

`ShareController::DownloadInfo()` 的请求接收、路径 DTO 校验失败、令牌与路径分享码不匹配、业务失败和元数据查询成功五条事件必须分别只记录固定完整摘要 `Received share download info request`、`Share download info request parameter validation failed`、`Share token does not match requested share`、`Get share download info failed` 与 `Share download info successful`。message 不得追加客户端 IP/端口、路径或解析后的分享 ID/码与 file ID、DTO/下游错误消息、令牌属性中的分享码或内部分享 ID、文件名/大小/hash/MIME/range 支持状态、Authorization、`X-Share-Token`、原始 access/share token 或其他账户/请求/响应值；调用方 request/instance/`download` 上下文、原 INFO/WARN/WARN/ERROR/INFO 级别和分支触发位置保持不变。

既有上下文创建、`DownloadShareRequest::FromPath()` 解析及错误透传、`ShareAuthFilter` 注入的内部分享 ID/分享码属性读取、令牌分享码与路径分享码绑定校验、`ShareService::GetDownloadInfo()` 调用及上下文传播、业务错误原样返回、`DownloadInfoResponse` 的 file_id/filename/file_size/file_hash/mime_type/supports_range 六字段映射和成功响应不得改变。令牌缺失/无效/scope 拒绝、取消/过期令牌实时拒绝、跨分享令牌拒绝、分享状态/权限与文件归属校验、完整性元数据以及 info 请求不增加文件或分享下载计数的行为保持不变。验证必须新增源码合同锁定五条固定摘要、五类动态边界值排除和完整控制器数据流，并继续执行 ShareLogContext/ShareDto/ShareService GoogleTest、真实下载/令牌安全流、上传安全网、OpenSpec、完整构建及标准完整 CTest。其余两个分享控制器入口继续独立审计；Phase 10 与最终 Definition of Done 在其余诊断迁移、兼容退役和目标环境门禁完成前保持未勾选。

旧实现上的定向源码合同按预期为 0/1，共 10 个失败断言：五条固定完整摘要均缺失，请求端点/分享码/file ID、DTO 错误消息、令牌属性分享码/路径分享码、下游错误/分享码/file ID 和成功分享码/file ID/文件大小五类动态日志各命中一次；`download` 上下文、路径 DTO、认证过滤器属性读取、令牌与路径绑定校验、Service 调用、三类错误透传、六字段响应映射、成功响应和 INFO/WARN/WARN/ERROR/INFO 级别分布断言均通过。
...
### 15.238 分享保存到网盘控制器诊断脱敏记录（2026-08-06）

`ShareController::Save()` 的请求接收、DTO 校验失败、令牌与路径分享码不匹配、业务失败和保存成功五条事件必须分别只记录固定完整摘要 `Received save share items request`、`Save share items request parameter validation failed`、`Share token does not match requested share`、`Save share items failed` 与 `Save share items successful`。message 不得追加客户端 IP/端口、路径或解析后的分享 ID/码、DTO/下游错误消息、令牌属性中的分享码或内部分享 ID、owner/目标用户 ID、file/folder ID 或数量、目标文件夹、saved/saved_file/saved_folder 计数、Authorization、`X-Share-Token`、原始 access/share token 或其他账户/请求/响应值；调用方 request/instance/`share` 上下文、原 INFO/WARN/WARN/ERROR/INFO 级别和分支触发位置保持不变。

既有 owner JWT 认证与 `user_id` 属性、分享令牌实时验证与 download scope、共享 download 限流桶、`SaveShareItemsRequest::FromRequest()` 解析及错误透传、内部分享 ID/分享码属性读取、令牌分享码与路径分享码绑定校验、`ShareService::SaveToDrive()` 的 request/internal share/target user/上下文参数、业务错误原样返回、`SaveShareItemsResponse::ToJson()` 的 saved_count/saved_file_count/saved_folder_count 三字段映射和成功响应不得改变。owner 令牌缺失/无效、分享令牌缺失/无效/scope 拒绝、取消/过期令牌实时拒绝、跨分享令牌拒绝、分享状态/权限/关联成员与目标文件夹归属校验、递归复制计划、冲突跳过、配额预留/退款、内容引用、路径/层级、项目/父目录计数、事务提交/回滚、成功后缓存失效及公开响应行为保持不变。验证必须新增源码合同锁定五条固定摘要、五类动态边界值排除和完整控制器数据流，并继续执行 ShareLogContext/ShareDto/ShareService/ShareRateLimit GoogleTest、真实分享限流/令牌安全流、上传安全网、OpenSpec、完整构建及标准完整 CTest。十个分享控制器值日志边界完成收敛；Phase 10 与最终 Definition of Done 在其余诊断迁移、兼容退役和目标环境门禁完成前保持未勾选。

旧实现上的定向源码合同按预期为 0/1，共 10 个失败断言：五条固定完整摘要均缺失，客户端端点/分享码、DTO 错误消息、令牌属性分享码/路径分享码、下游错误/分享码/user ID 和成功 saved_count/分享码/user ID 五类动态日志各命中一次；`share` 上下文、INFO/WARN/WARN/ERROR/INFO 级别、DTO 与错误透传、owner/分享认证属性读取、令牌与路径绑定、四参数 Service 调用、错误响应、三字段结果映射和成功响应断言均通过。
...
### 15.241 管理员用户状态变更控制器诊断脱敏记录（2026-08-06）

`AdminController::ChangeUserStatus()` 的请求接收、DTO 校验失败、业务失败和状态变更成功四条事件必须分别只记录固定完整摘要 `Admin change user status request`、`Change status request validation failed`、`Failed to change user status` 与 `Admin change user status successful`。Controller message 不得追加客户端 IP/端口、原始或解析后的路径 user ID、目标或操作员 user ID、status、DTO/下游错误消息、Authorization、原始 access token/JWT/JTI/claim、密码、存储凭据或其他账户/请求/响应值；调用方 request/instance/`admin` 上下文、原 INFO/WARN/ERROR/INFO 级别和分支触发位置保持不变。

既有全局 JWT 认证、`AdminAuthFilter` 后 `AdminRateLimitFilter` 的路由顺序、操作员 user ID 属性读取、空路径 ID 与无效数字格式校验、`std::stoull()` 转换、`ChangeStatusRequest::FromRequest()` 的 status 必填与 0/1/2 范围校验、DTO 错误透传、`AdminService::GetInstance()` 和 `ChangeUserStatus(target_id, status, operator_id, log_context)` 调用、业务错误原样返回、禁止修改自身、缺失用户 `404/80002/User not found` 映射、状态写入、`login_attempts`/`locked_until` 重置、审计和成功响应不得改变；DTO 与 AdminService 自有事件继续作为独立边界审计。验证必须新增源码合同锁定四条固定摘要、四类动态边界值排除、路由过滤器顺序和完整控制器/DTO 数据流，更新真实安全网固定标记，并继续执行 AdminLogContext/AdminDto/AdminService/AdminAuth/AdminRateLimit GoogleTest、真实管理流、上传安全网、OpenSpec、完整构建及标准完整 CTest。其余十个核心管理员控制器入口继续独立审计；Phase 10 与最终 Definition of Done 在其余诊断迁移、兼容退役和目标环境门禁完成前保持未勾选。

旧实现上的定向源码合同按预期为 0/1，共 8 个失败断言：请求接收、DTO 校验失败、业务失败和状态变更成功四条固定完整摘要均缺失，客户端端点、DTO 错误消息、下游错误消息和成功 target user ID 四类动态日志各命中一次；INFO/WARN/ERROR/INFO 级别、上下文、路由与过滤器顺序、操作员属性、路径校验与转换、DTO 必填/范围校验、Service 四参数调用、错误透传和成功响应断言均通过。
...
### 15.243 管理员用户可用空间变更控制器诊断脱敏记录（2026-08-06）

`AdminController::ChangeUserAvailableSpace()` 的请求接收、DTO 校验失败、业务失败和可用空间变更成功四条事件必须分别只记录固定完整摘要 `Admin change user available space request`、`Change available space request validation failed`、`Failed to change user available space` 与 `Admin change user available space successful`。Controller message 不得追加客户端 IP/端口、原始或解析后的路径 user ID、目标或操作员 user ID、`available_space_g`、DTO/下游错误消息、返回用户的十二项字段、Authorization、原始 access token/JWT/JTI/claim、密码、存储凭据或其他账户/请求/响应值；调用方 request/instance/`admin` 上下文、原 INFO/WARN/ERROR/INFO 级别和分支触发位置保持不变。

既有全局 JWT 认证、`AdminAuthFilter` 后 `AdminRateLimitFilter` 的路由顺序、操作员 user ID 属性读取、空路径 ID 与无效数字格式校验、`std::stoull()` 转换、`ChangeAvailableSpaceRequest::FromRequest()` 的必填无符号整数与 GiB 换算溢出校验、DTO 错误透传、`AdminService::GetInstance()` 和 `ChangeUserAvailableSpace(target_id, available_space_g, operator_id, log_context)` 调用、业务错误原样返回、禁止修改自身、缺失用户 `404/80002/User not found` 映射、Service 两层溢出保护、`storage_used + storage_reserved + available_space_bytes` 配额公式、配额写入、`admin.user.available_space_set` 审计、`data.user` 十二字段映射和成功响应不得改变；DTO 与 AdminService 自有事件继续作为独立边界审计。验证必须新增源码合同锁定四条固定摘要、四类动态边界值排除、路由过滤器顺序和完整控制器/DTO/响应数据流，在真实安全网加入自身可用空间修改关联，并继续执行 AdminLogContext/AdminDto/AdminService/AdminAuth/AdminRateLimit GoogleTest、真实管理流、上传安全网、OpenSpec、完整构建及标准完整 CTest。其余八个核心管理员控制器入口继续独立审计；Phase 10 与最终 Definition of Done 在其余诊断迁移、兼容退役和目标环境门禁完成前保持未勾选。

旧实现上的定向源码合同按预期为 0/1，共 8 个失败断言：请求接收、DTO 校验失败、业务失败和可用空间变更成功四条固定完整摘要均缺失，客户端端点、DTO 错误消息、下游错误消息和成功 target user ID 四类动态日志各命中一次；INFO/WARN/ERROR/INFO 级别、上下文、路由与过滤器顺序、操作员属性、路径校验与转换、DTO 必填无符号整数/GiB 溢出校验、Service 四参数调用、错误透传、`data.user` 与十二字段响应映射断言均通过。

实现后定向源码合同 1/1、全部 Admin GoogleTest 87/87、控制器/Service/可用空间 DTO 聚焦 GoogleTest 9/9、Admin/ChangeUserAvailableSpace/ChangeAvailableSpaceRequest 聚焦 CTest 94/94、真实管理流 25/25、上传安全网 898/898、OpenSpec 24/24、Python 语法、差异检查和完整后端构建通过。受管安全网确认自身可用空间修改继续被拒绝，Controller 固定请求/失败摘要与 Service 请求/拒绝事件、HTTP 完成事件共享同一 request/instance/`admin` 关联且四个所有权字段为空。标准完整 CTest 共 1574 项：1567 项通过、PgBouncer/Prometheus、3 项 S3 与 2 项分布式目标环境门控共 7 项跳过、0 失败，总耗时 543.51 秒。源码审计确认可用空间变更方法区段内四条固定摘要各 1 处，客户端端点、DTO 错误消息、下游错误消息和成功 target user ID 四类旧动态日志均为 0；上下文、级别、路由/过滤器、操作员属性、路径/DTO 校验、Service 四参数、错误透传、自身保护、缺失用户映射、两层溢出保护、配额公式与写入、审计、十二字段响应和公开行为保持不变。其余八个核心管理员控制器入口继续独立审计；Phase 10 与最终 Definition of Done 保持未勾选。

### 15.244 管理员用户软删除控制器诊断脱敏记录（2026-08-06）

`AdminController::SoftDeleteUser()` 的请求接收、业务失败和软删除成功三条事件必须分别只记录固定完整摘要 `Admin soft delete user request`、`Failed to soft delete user` 与 `Admin soft delete user successful`。Controller message 不得追加客户端 IP/端口、原始或解析后的路径 user ID、目标或操作员 user ID、下游错误消息、Authorization、原始 access token/JWT/JTI/claim、密码、存储凭据或其他账户/请求/响应值；调用方 request/instance/`admin` 上下文、原 INFO/ERROR/INFO 级别和分支触发位置保持不变。

既有全局 JWT 认证、`AdminAuthFilter` 后 `AdminRateLimitFilter` 的路由顺序、操作员 user ID 属性读取、空路径 ID 与无效数字格式校验、`std::stoull()` 转换、`AdminService::GetInstance()` 和 `SoftDeleteUser(target_id, operator_id, log_context)` 调用、业务错误原样返回、禁止删除自身、缺失用户 `404/80002/User not found` 映射、数据库失败的既有 500、用户 status 置为 0、用户更新、`admin.user.soft_delete` 审计和成功响应不得改变；AdminService 自有请求、拒绝、缺失、失败与成功事件继续作为独立边界审计。验证必须新增源码合同锁定三条固定摘要、三类动态边界值排除、路由过滤器顺序和完整控制器数据流，在真实安全网加入自身软删除关联，并继续执行 AdminLogContext/AdminService/AdminAuth/AdminRateLimit GoogleTest、真实管理流、上传安全网、OpenSpec、完整构建及标准完整 CTest。其余七个核心管理员控制器入口继续独立审计；Phase 10 与最终 Definition of Done 在其余诊断迁移、兼容退役和目标环境门禁完成前保持未勾选。

旧实现上的定向源码合同按预期为 0/1，共 6 个失败断言：请求接收、业务失败和软删除成功三条固定完整摘要均缺失，客户端端点、下游错误消息和成功 target user ID 三类动态日志各命中一次；INFO/ERROR/INFO 级别、上下文、路由与过滤器顺序、操作员属性、路径校验与转换、Service 三参数调用、错误透传和成功响应断言均通过。
...
### 15.246 管理员分享列表控制器诊断脱敏记录（2026-08-06）

`AdminController::ListShares()` 的请求接收、DTO 校验失败、业务失败和列表成功四条事件必须分别只记录固定完整摘要 `Admin list shares request`、`List shares request validation failed`、`Failed to list shares` 与 `Admin list shares successful`。Controller message 不得追加客户端 IP/端口、page/page_size/status/user_id/username 筛选或分页值、DTO/下游错误消息、操作员/分享/用户/文件 ID、列表项、分页/计数、审计详情、Authorization、原始 access token/JWT/JTI/claim、密码、存储凭据或其他账户/请求/响应值；调用方 request/instance/`admin` 上下文、原 INFO/WARN/ERROR/INFO 级别和分支触发位置保持不变。

既有全局 JWT 认证、`AdminAuthFilter` 后 `AdminRateLimitFilter` 的路由顺序、`ListSharesRequest::FromRequest(request, log_context)` 校验与错误原样返回、操作员 user ID 属性读取、`AdminService::GetInstance()` 和 `ListShares(*parse_result, operator_id, log_context)` 调用、业务错误透传、无文件活动分享停用、三类筛选、参数化用户名模糊匹配、计数与创建时间倒序分页、首文件与可空字段映射、`admin.share.list` 真实管理员读取审计、items/pagination JSON 映射和成功响应不得改变；Admin DTO 与 AdminService 自有事件继续作为独立边界审计。验证必须新增源码合同锁定四条固定摘要、三类动态边界值排除、路由过滤器顺序和完整 Controller/DTO/Service/响应数据流，在真实安全网加入非法分享列表校验关联，并继续执行 AdminLogContext/AdminDto/AdminService/AdminAuth/AdminRateLimit GoogleTest、真实管理流、上传安全网、OpenSpec、完整构建及标准完整 CTest。其余五个核心管理员控制器入口继续独立审计；Phase 10 与最终 Definition of Done 在其余诊断迁移、兼容退役和目标环境门禁完成前保持未勾选。

旧实现上的定向源码合同按预期为 0/1，共 6 个失败断言：请求接收、DTO 校验失败和业务失败三条固定完整摘要均缺失，客户端端点、DTO 错误消息和下游错误消息三类动态日志各命中一次；既有成功固定摘要、INFO/WARN/ERROR/INFO 级别、上下文、路由与过滤器顺序、DTO 五类查询解析、操作员属性、Service 三参数调用、两类错误透传、items/pagination 响应映射和成功响应断言均通过。

实现后定向源码合同 1/1、`Admin*` GoogleTest 81/81、Controller/Service/请求 DTO/响应 DTO 聚焦 GoogleTest 17/17、Admin/ListSharesRequest/ShareListResponse 聚焦 CTest 106/106、真实管理流 25/25、上传安全网 906/906、OpenSpec 24/24、Python 语法、差异检查和完整后端构建通过。受管安全网确认非法分享列表的 DTO/Controller/HTTP 事件共享同一 request/instance/`admin` 关联且四个所有权字段为空。标准完整 CTest 共 1577 项：1570 项通过、PgBouncer/Prometheus、3 项 S3 与 2 项分布式目标环境门控共 7 项跳过、0 失败，总耗时 555.32 秒。源码审计确认四条固定摘要各 1 处，客户端端点、DTO 错误消息和下游错误消息三类旧动态日志均为 0；上下文、级别、路由/过滤器、DTO 查询解析、操作员属性、Service 三参数、两类错误透传、停用/筛选/计数/倒序分页/字段映射、真实管理员读取审计、items/pagination JSON 和公开行为保持不变。其余五个核心管理员控制器入口继续独立审计；Phase 10 与最终 Definition of Done 保持未勾选。

### 15.247 管理员分享详情控制器诊断脱敏记录（2026-08-06）

`AdminController::GetShareDetail()` 的请求接收、业务失败和详情成功三条事件必须分别只记录固定完整摘要 `Admin get share detail request`、`Failed to get share detail` 与 `Admin get share detail successful`。Controller message 不得追加客户端 IP/端口、原始或解析后的路径 share ID/code、操作员/分享/用户/文件 ID、下游错误消息、十项返回字段、审计详情、Authorization、原始 access token/JWT/JTI/claim、密码、存储凭据或其他账户/请求/响应值；调用方 request/instance/`admin` 上下文、原 INFO/ERROR/INFO 级别和分支触发位置保持不变。

既有全局 JWT 认证、`AdminAuthFilter` 后 `AdminRateLimitFilter` 的路由顺序、空 share ID 校验与 `Missing required parameter: share_id` 错误、外部字符串 `share_code` 原样传播且不作数字转换、操作员 user ID 属性读取、`AdminService::GetInstance()` 和 `GetShareDetail(share_id, operator_id, log_context)` 调用、业务错误透传、无文件活动分享停用、按 `share_code` 查询、`AdminShareNotFound` 映射、首文件与可空字段、PostgreSQL 布尔读取、`admin.share.detail` 真实管理员读取审计、`ShareDetailResponse::ToJson()` 十字段直接映射和成功响应不得改变；AdminService 自有请求、缺失、数据库失败与成功事件继续作为独立边界审计。验证必须新增源码合同锁定三条固定摘要、三类动态边界值排除、路由过滤器顺序和完整 Controller/Service/响应数据流，更新真实安全网缺失分享详情的固定标记，并继续执行 AdminLogContext/AdminDto/AdminService/AdminAuth/AdminRateLimit GoogleTest、真实管理流、上传安全网、OpenSpec、完整构建及标准完整 CTest。其余四个核心管理员控制器入口继续独立审计；Phase 10 与最终 Definition of Done 在其余诊断迁移、兼容退役和目标环境门禁完成前保持未勾选。

旧实现上的定向源码合同按预期为 0/1，共 6 个失败断言：请求接收、业务失败和详情成功三条固定完整摘要均缺失，客户端端点、下游错误消息和成功 share ID 三类动态日志各命中一次；既有 INFO/ERROR/INFO 级别、上下文、路由与过滤器顺序、空路径错误、无数字转换、操作员属性、Service 三参数调用、错误透传、十字段直接响应映射和成功响应断言均通过。

实现后定向源码合同 1/1、`Admin*` GoogleTest 82/82、Controller/Service/响应 DTO 聚焦 GoogleTest 5/5、Admin/GetShareDetail/ShareDetailResponse 聚焦 CTest 96/96、真实管理流 25/25、上传安全网 906/906、OpenSpec 24/24、Python 语法、差异检查和完整后端构建通过。受管安全网确认缺失分享详情的 Controller/Service/HTTP 事件共享同一 request/instance/`admin` 关联且四个所有权字段为空。标准完整 CTest 共 1578 项：1571 项通过、PgBouncer/Prometheus、3 项 S3 与 2 项分布式目标环境门控共 7 项跳过、0 失败，总耗时 543.56 秒。源码审计确认三条固定摘要各 1 处，客户端端点、下游错误消息和成功 share ID 三类旧动态日志均为 0；上下文、级别、路由/过滤器、空路径错误、外部字符串标识、操作员属性、Service 三参数、错误透传、停用/查询/缺失映射、十字段直接映射、真实管理员读取审计和公开行为保持不变。其余四个核心管理员控制器入口继续独立审计；Phase 10 与最终 Definition of Done 保持未勾选。

### 15.248 管理员强制取消分享控制器诊断脱敏记录（2026-08-06）

`AdminController::ForceCancelShare()` 的请求接收、业务失败和取消成功三条事件必须分别只记录固定完整摘要 `Admin force cancel share request`、`Failed to force cancel share` 与 `Admin force cancel share successful`。Controller message 不得追加客户端 IP/端口、原始或解析后的路径 share ID/code、操作员/分享/用户/文件 ID、下游错误消息、状态或审计详情、Authorization、原始 access token/JWT/JTI/claim、密码、存储凭据或其他账户/请求/响应值；调用方 request/instance/`admin` 上下文、原 INFO/ERROR/INFO 级别和分支触发位置保持不变。

既有全局 JWT 认证、`AdminAuthFilter` 后 `AdminRateLimitFilter` 的 DELETE 路由顺序、操作员 user ID 属性读取、空 share ID 校验与 `Missing required parameter: share_id` 错误、外部字符串 `share_code` 原样传播且不作数字转换、`AdminService::GetInstance()` 和 `ForceCancelShare(share_id, operator_id, log_context)` 调用、业务错误透传、按 `share_code` 查询、`AdminShareNotFound` 与已取消分享映射、按内部 ID 将状态置零并更新时间、`admin.share.force_cancel` 四项详情审计和空成功响应不得改变；AdminService 自有请求、缺失、已取消、数据库失败与成功事件继续作为独立边界审计。验证必须新增源码合同锁定三条固定摘要、三类动态边界值排除、路由过滤器顺序和完整 Controller/Service 数据流，在真实安全网加入缺失分享强制取消关联，并继续执行 AdminLogContext/AdminService/AdminAuth/AdminRateLimit GoogleTest、真实管理流、上传安全网、OpenSpec、完整构建及标准完整 CTest。其余三个核心管理员控制器入口继续独立审计；Phase 10 与最终 Definition of Done 在其余诊断迁移、兼容退役和目标环境门禁完成前保持未勾选。

旧实现上的定向源码合同按预期为 0/1，共 6 个失败断言：请求接收、业务失败和取消成功三条固定完整摘要均缺失，客户端端点、下游错误消息和成功 share ID 三类动态日志各命中一次；既有 INFO/ERROR/INFO 级别、上下文、DELETE 路由与过滤器顺序、操作员属性、空路径错误、无数字转换、Service 三参数调用、错误透传、查询/缺失/已取消映射、状态与更新时间写入、四项详情审计和空成功响应断言均通过。
