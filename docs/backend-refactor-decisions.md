...
This note records accepted product and policy decisions together with their current implementation status for the completed backend refactor roadmap. The closed roadmap is archived at [`docs/archive/2026-07-14-backend-refactor-todo.md`](archive/2026-07-14-backend-refactor-todo.md); client closure work is tracked in [`docs/TODO.md`](TODO.md), while the active distributed-backend roadmap is tracked in the repository-root [`TODO.md`](../TODO.md).
...
| Object storage compatibility | S3/MinIO-compatible object storage is supported as a configurable runtime backend while local filesystem remains available for development and compatibility drain | Implemented with independent final/staging selectors, persisted upload descriptors, S3-native staging and durable cleanup/reconciliation jobs | Closed for implementation; production cutover evidence and compatibility retirement remain in the distributed roadmap |
...
**Current implementation behavior:** The runtime has independent `storage_backend` and `upload_staging_backend` selectors. `UploadStagingStorage` provides local and S3 implementations for upload-session chunks and assembled objects; `BlobStore` provides local and S3 implementations for final content blobs and Range reads. Each non-instant upload persists its selected staging backend and exact session prefix, so later chunk, complete, cancel, cleanup, and reconciliation operations follow the task descriptor rather than the process default. The final distributed profile uses S3 for both selectors; local storage remains available for single-process development and controlled compatibility drain.

**Accepted target behavior:** The original decision introduced S3/MinIO as a configurable final-blob backend while preserving the local filesystem backend and separating upload staging, final blob storage, and database lifecycle decisions. ADR-002 subsequently extended the same boundary to S3-native staging without changing the public upload API. Storage code must continue avoiding local-only primitives where object storage cannot provide the same guarantee.

**Implemented S3-native staging responsibilities:** Chunks use an upload-session namespace and conditional writes, so an exact retry is idempotent while conflicting bytes are rejected. Assembly streams the persisted chunk descriptors into a versioned multipart staging object and returns MD5/SHA-256 metadata for final validation. Completed, cancelled, and expired sessions enqueue durable `staging_cleanup` work; interrupted multipart operations are journaled as `multipart_abort` jobs. Cleanup always targets the persisted session prefix and is safe to retry.

**Final blob-storage responsibilities for S3/MinIO:** `BlobStore` owns content-addressed SHA-256 final keys, promotion from a staging descriptor, existence and size checks, bounded Range reads, inventory for reconciliation, and idempotent deletion. Callers consume blob descriptors and never depend on POSIX paths or atomic filesystem rename.

**Consistency and compensation model:** Database transactions and object-store side effects remain separate failure domains. Complete first claims a PostgreSQL finalize lease, performs assembly and promotion outside the final transaction, renews the lease, then atomically creates the file/content reference, transfers reserved quota, marks the upload completed, and enqueues `staging_cleanup`. If promotion succeeds but that transaction fails, the content-addressed final object is retained for an idempotent retry or reconciliation rather than being deleted while the client result is unknown. If the transaction commits, a lease-backed Worker removes staging asynchronously. Permanent deletion decrements `ref_count` in PostgreSQL and enqueues `blob_gc`; the Worker rechecks zero references before deleting the final object. Missing objects remain successful cleanup, and retry exhaustion is persisted for diagnosis or recovery.
...
**Implementation impact:** Storage/download boundaries keep database state transitions explicit around object-store promotion and deletion, use persisted staging and blob descriptors so controllers and Workers do not need local path knowledge, and support the same backend binary in local/local and S3/S3 profiles. AWS SDK for C++ remains a mandatory build dependency, including local-only runtime configurations; making it optional would require a separate conditional-build design and is not an active follow-up.

**Follow-up status:** The storage-boundary and ADR-002 S3 staging implementation are closed. Target-environment rollout evidence, legacy local-state drain, and compatibility-code retirement remain explicitly tracked in the repository-root distributed roadmap; they are deployment gates rather than missing storage abstractions.
