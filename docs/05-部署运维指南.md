...
ExecStart=/opt/disk/bin/disk -c /opt/disk/config/config.json
...
# 安装服务
nssm install Disk "C:\disk\bin\disk.exe" "-c C:\disk\config\config.json"
...
2. **检查配置文件**
   ```bash
   # 验证 JSON 格式
   jq . /opt/disk/config/config.json
   ```
