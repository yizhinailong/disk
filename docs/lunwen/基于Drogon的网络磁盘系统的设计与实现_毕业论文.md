...
该截图用于展示用户通过账号密码完成登录后，系统返回统一 JSON 响应、用户信息、Access Token 与 Refresh Token 的实际运行效果，同时可体现后续业务接口依赖令牌鉴权的基础流程。

账号模块围绕注册、登录、令牌刷新、登出和密码修改等接口实现。系统在控制器层接收 JSON 请求并交由 DTO 完成参数校验，服务层通过 MySQL 查询用户账号状态、邮箱和用户名唯一性，并使用 libsodium 提供的 Argon2id 算法保存密码哈希。用户登录成功后，后端签发 Access Token 与 Refresh Token，其中刷新令牌及其轮换状态保存于 Redis；登出时系统将访问令牌加入黑名单并删除刷新令牌，保证登录态可以被及时撤销。

### 5.2 文件模块功能实现

图5-2 文件分片上传与上传完成运行截图

[程序功能的截图]

该截图用于展示文件上传初始化、分片上传和完成合并后的接口响应或客户端结果，重点体现上传任务 ID、已上传分片、秒传判断、文件元数据写入等运行状态。

图5-3 文件下载与断点下载运行截图

[程序功能的截图]

该截图用于展示用户下载文件或使用 Range 请求下载文件片段的运行结果，重点体现文件流返回、响应状态码、Content-Range 等断点下载相关信息。

文件模块由文件列表、文件上传、文件下载、重命名、移动、复制、搜索和软删除等能力组成。上传实现采用初始化、分片上传、完成合并和取消上传四阶段流程：初始化阶段进行配额预检查和内容哈希查重，若 `file_contents` 中已存在相同内容则直接创建文件元数据完成秒传；否则创建 `upload_tasks` 记录并预占用空间。分片上传阶段将分片写入临时目录并记录到 `upload_task_chunks`，完成阶段通过 `AssemblyWorkerPool` 合并分片、校验哈希并写入内容对象和文件元数据。下载接口根据文件归属和权限校验结果读取本地内容寻址文件，并支持 HTTP Range 断点下载。
...
该截图用于展示用户查询文件列表、创建目录、查看目录树或面包屑路径的运行效果，重点体现文件和目录混合展示、分页信息以及目录层级导航结果。

图5-5 文件搜索、重命名与移动复制运行截图

[程序功能的截图]

该截图用于展示文件搜索、重命名、批量移动或复制等文件管理操作的接口响应或客户端变化，重点体现操作完成后文件元数据和目录位置的更新结果。

目录模块基于 `folders` 表中的 `parent_id` 字段维护树形结构，创建目录时校验父目录归属和同级名称唯一性，目录树与面包屑接口则根据父子关系构造层级数据。删除文件或目录时系统不立即物理移除数据，而是将恢复所需的原路径、项目类型、内容引用和备份信息写入 `trash` 表；用户可以在回收站中执行恢复、彻底删除或清空操作。彻底删除时服务层维护 `file_contents.ref_count`，只有引用计数归零时才释放物理文件，从而保证去重存储的一致性。
...
该截图用于展示用户为文件或目录创建分享链接后的运行结果，重点体现分享短码、有效期、访问权限、访问密码设置和分享记录返回信息。

图5-7 访客访问分享与下载运行截图

[程序功能的截图]

该截图用于展示访客通过分享密码获取 Share Token、浏览分享内容和下载分享文件的运行效果，重点体现公开访问流程与分享令牌鉴权机制。

分享模块通过 `shares` 和 `share_files` 两张表保存分享短码、分享者、权限、访问密码哈希、有效期和被分享项目。用户创建分享时，系统生成高随机性的分享短码并写入关联项目；访客访问公开分享时，后端先校验分享状态、过期时间和可选访问密码，再签发短期 Share Token。后续浏览分享内容和下载分享文件时，访客需通过 `X-Share-Token` 请求头携带该令牌，分享过滤器负责校验公开访问边界，避免用户 JWT 与访客分享令牌混用。

图5-8 回收站列表与恢复运行截图

[程序功能的截图]

该截图用于展示用户查看回收站、恢复文件、彻底删除或清空回收站的运行效果，重点体现误删恢复、命名冲突处理和彻底删除后的状态变化。
...
该截图用于展示管理员查看用户列表、修改用户状态或角色的运行结果，重点体现管理员接口的角色权限控制和用户账号状态变更。

图5-10 系统状态与存储统计运行截图

[程序功能的截图]

该截图用于展示管理员查看系统概览、数据库连接、Redis 状态、磁盘空间和全平台存储统计的运行结果，体现系统运维监控能力。

管理员模块在普通 JWT 鉴权之后增加角色校验，只有管理员账号可以访问用户管理、用户文件查看、全局存储统计、分享强制取消和系统状态等接口。系统状态接口聚合数据库连接、Redis 状态、磁盘空间和服务运行时间等信息；操作日志模块记录上传、下载、删除、分享等关键行为，为问题排查、安全审计和后续运维统计提供数据基础。
