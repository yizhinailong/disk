...
该截图展示了系统登录与注册页面的实际运行效果，包括用户名密码登录表单、邮箱注册表单以及登录成功后客户端获取并存储令牌的完整流程。

账号模块围绕注册、登录、令牌刷新、登出和密码修改等接口实现。系统在控制器层接收 JSON 请求并交由 DTO 完成参数校验，服务层通过 MySQL 查询用户账号状态、邮箱和用户名唯一性，并使用 libsodium 提供的 Argon2id 算法保存密码哈希。用户登录成功后，后端签发 Access Token 与 Refresh Token，其中刷新令牌及其轮换状态保存于 Redis；登出时系统将访问令牌加入黑名单并删除刷新令牌，保证登录态可以被及时撤销。客户端在登录页面完成认证后，将令牌持久化到本地，后续所有业务请求均通过 `Authorization` 请求头携带访问令牌；访问令牌过期时自动使用刷新令牌换取新令牌，刷新失败则跳转回登录页面。个人设置页面提供昵称修改和密码修改功能，同时展示当前用户存储空间使用情况。
...
该截图展示了文件浏览与管理页面的实际运行效果，包括文件和目录的混合列表展示、目录树面板、面包屑路径导航、文件搜索以及重命名、移动、复制、创建目录和删除等文件管理操作。

文件浏览页面是系统的核心交互页面，左侧目录树面板和面包屑导航组件基于 `folders` 表中的 `parent_id` 字段维护的树形结构构造层级数据。文件列表接口支持按名称、类型、大小和修改时间排序，并提供关键词搜索能力。创建目录时校验父目录归属和同级名称唯一性；重命名操作校验同级名称唯一性后更新文件元数据；移动和复制操作通过更新或创建文件元数据的父目录引用来实现，复制时共享内容对象并增加引用计数。删除文件或目录时系统不立即物理移除数据，而是将恢复所需的原路径、项目类型、内容引用和备份信息写入 `trash` 表，供回收站页面使用。客户端通过 QML ListView 组件展示文件列表，结合右键上下文菜单和工具栏按钮提供丰富的文件管理操作入口。
...
该截图展示了文件传输中心页面的实际运行效果，包括文件分片上传进度、上传任务队列、文件下载任务以及断点续传等功能。

文件上传采用初始化、分片上传、完成合并和取消上传四阶段流程：初始化阶段进行配额预检查和内容哈希查重，若 `file_contents` 中已存在相同内容则直接创建文件元数据完成秒传；否则创建 `upload_tasks` 记录并预占用空间。分片上传阶段将分片写入临时目录并记录到 `upload_task_chunks`，完成阶段通过 `AssemblyWorkerPool` 合并分片、校验哈希并写入内容对象和文件元数据。下载接口根据文件归属和权限校验结果读取本地内容寻址文件，并支持 HTTP Range 断点下载。传输中心页面实时展示上传和下载任务队列的状态、进度和错误信息，支持暂停、重试和清除已完成任务，为用户提供直观的传输管理体验。
...
该截图展示了分享管理页面的实际运行效果，包括创建分享链接、查看已创建的分享列表、访客通过分享码和密码访问共享文件以及下载分享内容等功能。

分享模块通过 `shares` 和 `share_files` 两张表保存分享短码、分享者、权限、访问密码哈希、有效期和被分享项目。用户创建分享时，系统生成高随机性的分享短码并写入关联项目；访客访问公开分享时，后端先校验分享状态、过期时间和可选访问密码，再签发短期 Share Token。后续浏览分享内容和下载分享文件时，访客需通过 `X-Share-Token` 请求头携带该令牌，分享过滤器负责校验公开访问边界，避免用户 JWT 与访客分享令牌混用。客户端分享页面以列表形式展示用户已创建的所有分享记录，支持查看分享详情、复制分享链接和取消分享等操作。
...
该截图展示了回收站页面的实际运行效果，包括已删除文件和目录列表、文件恢复操作、彻底删除操作以及清空回收站等功能。

用户可以在回收站页面查看已删除的文件和目录，执行恢复、彻底删除或清空操作。恢复时系统将项目还原到原路径，若原路径已不存在或存在同名冲突则进行相应处理；彻底删除时服务层维护 `file_contents.ref_count`，只有引用计数归零时才释放物理文件，从而保证去重存储的一致性。清空回收站操作批量处理所有已删除项目，逐项执行彻底删除逻辑并释放对应的存储空间。客户端回收站页面与文件浏览页面共享相似的列表展示组件，但操作按钮替换为恢复和彻底删除。
...
该截图展示了管理后台页面的实际运行效果，包括用户管理、分享管理和系统监控等功能标签页。

管理员模块在普通 JWT 鉴权之后增加角色校验，只有管理员账号可以访问用户管理、用户文件查看、全局存储统计、分享强制取消和系统状态等接口。用户管理标签页以分页列表形式展示所有注册用户，支持修改用户状态和角色分配；分享管理标签页展示全平台分享记录，管理员可强制取消违规分享；系统监控标签页聚合数据库连接状态、Redis 连接状态、磁盘空间使用情况和服务运行时间等信息，为运维人员提供系统健康度概览。操作日志模块记录上传、下载、删除、分享等关键行为，为问题排查、安全审计和后续运维统计提供数据基础。
