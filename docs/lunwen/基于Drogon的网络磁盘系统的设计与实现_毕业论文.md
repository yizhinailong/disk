...
在用户端的数据流中，普通用户首先提交账号和密码，后端认证模块完成身份校验后返回 Access Token 与 Refresh Token，其中刷新令牌、令牌黑名单和登录失败计数等短时状态写入 Redis；此后，用户携带访问令牌发起文件上传初始化请求，后端先校验用户配额，再根据文件 MD5 哈希查询 `file_contents` 表判断是否存在相同内容。如果已有内容存在，系统直接创建 `files` 元数据记录并增加内容引用计数，实现秒传；如果不存在，则创建 `upload_tasks` 上传任务并逐块接收分片，将分片状态记录到 `upload_task_chunks`，所有分片上传完成后由后端合并为完整文件，写入内容寻址存储目录，并在 `file_contents` 和 `files` 表中创建正式记录。当用户请求下载文件时，后端首先校验令牌和文件所有权，确认通过后从本地文件系统读取内容对象并以流式方式写入 HTTP 响应，同时支持 Range 断点下载。当用户删除文件或目录时，系统将项目元数据迁移到 `trash` 表保留恢复信息，彻底删除时再减少内容引用计数并按需清理物理文件。当用户生成分享链接时，后端在 `shares` 表创建分享记录，并通过 `share_files` 表关联被分享的文件或目录；未登录访客先通过公开访问接口校验分享短码、密码和过期状态，获取短期分享访问令牌后，再通过 `X-Share-Token` 浏览或下载分享内容。

在管理员端的数据流中，管理员登录后可以向后端发起用户查询、用户状态或角色修改、用户文件查看、存储统计汇总、分享审核和系统状态查询等请求。后端接收到管理员请求后，在 JWT 过滤器完成身份校验的基础上，再由管理员鉴权过滤器检查角色权限，确认为管理员角色后才执行相应的数据库查询或状态修改操作，并将处理结果以统一 JSON 响应返回调用方。

从整体数据流的角度来看，系统已经形成了"客户端或管理员提交请求、全局过滤器完成权限校验和限流、后端控制器转发请求、服务层处理业务逻辑、MySQL 保存结构化元数据、Redis 保存短时状态、本地文件系统保存内容对象、控制器返回统一 JSON 或文件流响应"这样的完整数据流转链路，符合 RESTful 后端服务的架构特征。
...
graph LR
    ...
    User -->|操作请求| Client
    Admin -->|管理操作| Client
    Client -->|HTTP请求| Filter
    Filter --> RateLimit
    RateLimit -->|鉴权通过| Auth
    RateLimit -->|鉴权通过| FileCtrl
    RateLimit -->|鉴权通过| DirCtrl
    RateLimit -->|鉴权通过| ShareCtrl
    RateLimit -->|鉴权通过| TrashCtrl
    RateLimit -->|管理员鉴权| AdminCtrl
    RateLimit -->|鉴权通过| LogCtrl
    Auth -->|读写用户数据| DB
    Auth -->|刷新令牌/黑名单/失败计数| Redis
    FileCtrl -->|读写元数据| DB
    FileCtrl -->|读写文件实体| FS
    DirCtrl -->|读写目录数据| DB
    ShareCtrl -->|读写分享记录| DB
    ShareCtrl -->|分享访问令牌/尝试计数| Redis
    TrashCtrl -->|读写回收站数据| DB
    AdminCtrl -->|查询统计数据| DB
    LogCtrl -->|读写日志/系统状态| DB
    LogCtrl -->|检查连接状态| Redis
    DB -->|返回结果| Client
    Redis -->|返回短时状态| Client
    FS -->|文件数据流| Client
```
