...
`DistributedTopologyContract` 必须区分环境门禁的注册库存与实际运行结果：CTest 继续注册 7 个可选门禁；当最近验证明确使用固定依赖启用其中 6 个时，`TODO.md` 顶部必须记录总库存 `1425`、通过 `1424`、仅 `DistributedFlowIntegration` 1 项因缺少 Docker 跳过，并继续保留目标 MinIO/云 S3 与多实例环境验收提示。不得把启用后通过的门禁仍计为跳过，也不得用本地进程拓扑把容器或目标环境门禁标记为完成。
