...
| `HealthService_test.cpp` / `HealthService.*` | UTC 时间戳格式化只供 `BuildBaseResult` 使用，必须留在 `HealthService.cpp` 匿名命名空间且不得由头文件声明为类成员；liveness/readiness 均输出长度 20、以 `Z` 结尾并可按 `%Y-%m-%dT%H:%M:%SZ` 完整解析的时间戳，跨平台 UTC 转换分支保留 | 源码内部链接合同 + 纯状态健康行为单测；角色依赖、健康字段、错误脱敏和响应不变 |
