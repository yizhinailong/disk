...
| `MetricsService_test.cpp` | HTTP operation/status 标签归一化；分片成功数量/字节 counter；活动上传 gauge；完成六阶段名称与累计直方图桶；finding 类型固定映射与未知类型归并；Prometheus 转义；数据库失败标记；禁止高基数标签 | 无网络单测 + 可注入快照 |
