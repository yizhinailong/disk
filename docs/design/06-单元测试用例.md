...
| `StorageWorkerRuntime_test.cpp` / `test_worker_drain_takeover.py` / `test_distributed_topology.py` | drain 幂等停止后续 poll；真实 Worker A 持有被行锁阻塞的任务时收到 SIGTERM，先退出 readiness 且不认领新哨兵，超时退出后保留租约；Worker B 先处理哨兵，再严格晚于 PostgreSQL 租约截止接管目标；Compose 终止宽限大于应用 drain | C++ 运行时单测 + 唯一临时 PostgreSQL/真实双 Worker + Compose 静态合同；不直接更新租约或任务状态 |
