...
| `ConfigMgr_test.cpp` / `ProcessRuntime_test.cpp` / `StorageWorkerRuntime_test.cpp` / `test_secure_local_staging_cutoff.py` / `test_worker_drain_takeover.py` / `test_distributed_topology.py` | 实例注册前 bootstrap 与 ConfigMgr 分别使用固定 operation、空 instance/所有权与有界失败摘要；配置消息不泄露路径、endpoint、凭据或 secret；drain 幂等停止后续 poll；进程与 Worker 运行时事件使用实际 instance 与空请求/任务所有权；真实安全模式 API/local staging 在监听前以退出码 1 拒绝并输出合规 bootstrap/config 摘要；真实 Worker A 收到 SIGTERM 后退出 readiness、不认领新哨兵，超时退出后保留租约并由 B 自然接管；随后 B/C 独占竞争 | C++ 源码合同 + 内存 NDJSON/运行时单测 + 真实启动拒绝 + 唯一临时 PostgreSQL/三个真实 Worker 进程（任一竞争阶段同时两个）+ Compose 静态合同；不直接更新任务状态或租约 |
...
| `StorageFactory_test.cpp` / `ProcessRuntime_test.cpp` / `FileServiceAtomicity_test.cpp` / `LocalBlobStore_test.cpp` / `S3ObjectStorage_test.cpp` / `DownloadResponder_test.cpp` | 工厂 bundle 与 `StorageMgr` 直接使用 `UploadStagingStorage`，ApplicationContext/上传服务只接收暂存和 Blob 能力；仓库不存在空 `IFileStorage`、`GetStorage()`、无用透传成员、三个 manager 静态初始化探针、`LocalFileStorage::DeletePath`、`IBlobStore::Exists(path)`、`OpenForRead(path)`、`OpenBlobForRead`、公开 `FileStorageReadStream` 或 `GetFinalStoragePath(hash)`；原子性模拟器通过 `DiscardAssembly` 清理组装物，local/S3 最终对象存在性通过 `BlobExists(BlobDescriptor)` 检查，流式下载通过 `OpenBlobRangeForRead(BlobDescriptor, start, length)`，提升结果直接验证实际 SHA-256 locator；`ProcessRuntimeState` 实例初始化状态、本地大文件快速路径与描述符兼容路由保留 | C++ 源码合同 + 工厂类型断言 + local/S3 存储、提升与下载行为回归；不删除 readiness、持久 Blob GC 或描述符兼容路由 |
...
| `ContentService_test.cpp` / `test_safety_content_quota.py` | 8 个公开查找/引用获取/单批增减/Blob GC 入队入口按值接收默认空上下文，内部 reference gate 及 UploadLifecycle/FileMutation/Share/Trash 全部保留原上下文；10 条操作失败事件固定，无 content/file/user ID、hash、size/delta、path/MIME、batch/affected rows、GC payload、SQL、domain error/异常正文或字段推断，构造事件保持空请求/任务关联；真实 `file_contents` UPDATE trigger 使秒传引用递增失败，核对 HTTP 500/`10006`、request/instance/`upload_init`、四个空所有权字段、无空 request 重复/application 异常正文，并保持无目标文件/任务、ref_count、quota、内容行和 Blob 不变；SQL、行锁、reference gate、GC 入队、可选值/集合/Result 和公开响应不变 | 源码合同 GoogleTest + 真实 HTTP/PostgreSQL/受管进程 NDJSON |
