...
| `test_local_staging_migration.py` | V002 在原卷创建三项 local 进行中任务；迁移后同卷 current API/唯一 Worker 分别完成、取消、过期；验证三种终态、配额转换/释放、cleanup job 成功、原卷工件清零与未挂载目录哨兵不变 | 固定旧/当前二进制 + 唯一临时 PostgreSQL + Redis + 原卷/未挂载目录隔离；不宣称任意节点恢复 local 暂存 |
