...
| `FileRepository_test.cpp` / `FileServiceUtils.*` / `FolderService.cpp` | 共享 `ExtractFileExtension` 覆盖无点、末尾点、单点、多点和大小写原样后缀；上传秒传/完成两个元数据写入点与文件重命名唯一写入点必须直接调用该 helper，两个 service 不保留本地同名实现。回收站恢复冲突名解析保持专用。只被 `BuildFolderSnapshot` 调用的 `DateToJson` 必须留在 `.cpp` 匿名命名空间，头文件不声明该符号或直接依赖 `trantor::Date`；root/folders/files 的 created_at/updated_at 继续逐项使用 `toDbStringLocal()`。`FolderService.cpp` 的文件夹创建/重命名和后代文件路径更新必须直接使用共享 `BuildFolderPath`/`BuildFilePath`，不保留两个匿名副本；直接行为覆盖根目录与嵌套目录下的文件/文件夹路径 | 直接行为 GoogleTest + 集中源码合同；文件名校验、名称、路径、快照 JSON、事务、后代更新、计数、错误、响应和回收站恢复语义不变 |
