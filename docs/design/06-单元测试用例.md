...
| 频率限制 | 登录/API 频率；注册 IP、上传路由与所有者下载路由限流日志关联；分享 access/browse/download 边界、隔离、认证和 fail-open | `LoginRateLimit_test.cpp`、`RegisterRateLimit_test.cpp`、`RegisterRateLimitLogContext_test.cpp`、`UploadRateLimitLogContext_test.cpp`、`DownloadRateLimit_test.cpp`、`DownloadRateLimitLogContext_test.cpp`、`ShareRateLimitFilter_test.cpp`、`test_safety_upload_invariants.py`、`test_upload_rate_limit.py`、`test_download_flow.py`、`test_share_rate_limit.py` | P0 |
