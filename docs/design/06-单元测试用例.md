...
| `DtoBase_test.cpp` / `test_safety_upload_invariants.py` | 12 个有生产调用的共享解析 helper 只返回既有 `Result<T>`/`ErrorInfo`，无 `LogHelper` 依赖或日志调用；源码合同拒绝只由单测调用的 `RequireBool`/`OptionalInt` 回归，其余行为覆盖无效 JSON、缺失/错误类型标量、非法路径/查询整数、错误/空/超长 ID 数组的错误码与规范化消息并以内存 sink 验证零事件；真实空 `trash_ids` 恢复失败继续由显式上下文边界记录，且不存在同消息的空 request 重复事件 | 行为/源码/内存 sink GoogleTest + 真实 HTTP/受管进程 NDJSON |
