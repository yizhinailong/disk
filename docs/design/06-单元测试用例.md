...
| `QuotaService_test.cpp` / `StorageReconciliationService_test.cpp` / `test_safety_upload_invariants.py` | 11 个活跃 standalone/transaction-client 配额写入入口按值接收默认空上下文，17 条操作事件全部使用调用方上下文且构造事件保持空请求/任务关联；编译期/源码合同拒绝两个无调用 `GetReconciliation` 形状、专属结构和 SQL 回归，保留内部 overload 与 UploadLifecycle/FileMutation/Trash 生产调用；`StorageReconciliationService` 继续分页检测并持久化/消解 used/reserved 配额 finding；真实 upload init 在 quota=used+reserved 时保持 HTTP 400/`50004`、零新任务/预留，helper 与上层固定拒绝事件核对 request/instance/`upload_init` 及四个空所有权字段；日志不含 user/bytes/delta、SQL、连接、domain error、数据库异常、JWT/凭据，不从参数、行数或错误推断字段；活跃 SQL、条件更新、事务和错误语义不变 | 编译期/源码合同 GoogleTest + 真实 HTTP/PostgreSQL/受管进程 NDJSON |
