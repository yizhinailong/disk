...
| `FileRepository_test.cpp` / `FileMutationServiceMove_test.cpp` / `FileListCache_test.cpp` / `test_safety_content_quota.py` / `test_safety_move_copy_path.py` | 文件仓储保留按 ID/用户归属读取、批量读取、重命名、位置与逐文件路径更新原语；全部 9 个活跃方法显式接收 standalone client 或当前 transaction，编译期合同要求仓储可默认构造且为空类型，源码合同拒绝无读取 `m_db_client`/注入构造、无调用 `UpdateDescendantFilePathsForFolderMove`、专属路径前缀 SQL 和事务外 `IsFilenameExists` 回归；文件重命名在同一 `TransactionRunner` 中 `FOR UPDATE` 读取目标，获取独立文件名称锁，排除自身检查冲突，读取文件夹路径并检查更新结果，`Run` 成功返回后才失效列表缓存；8 个并发同名重命名只有一个成功，其余稳定返回 `409/50007`，名称与路径保持一致。`FolderService` 重命名与 `FileMutationService` 移动继续在事务中逐文件调用 `UpdateFilePath`，真实嵌套移动对账根/后代文件夹和后代文件路径 | 编译期/源码合同 GoogleTest + 真实 HTTP/PostgreSQL/并发安全网；移动事务、路径结果和 item count 不变 |
