...
| `StorageFactory_test.cpp` / `ProcessRuntime_test.cpp` / `FileServiceAtomicity_test.cpp` / `S3ObjectStorage_test.cpp` | 工厂 bundle 与 `StorageMgr` 直接使用 `UploadStagingStorage`，ApplicationContext/上传服务只接收暂存和 Blob 能力；仓库不存在空 `IFileStorage`、`GetStorage()`、无用透传成员、三个 manager 静态初始化探针、`LocalFileStorage::DeletePath` 或 `IBlobStore::Exists(path)`；原子性模拟器通过 `DiscardAssembly` 清理组装物，local/S3 最终对象存在性通过 `BlobExists(BlobDescriptor)` 检查；`ProcessRuntimeState` 实例初始化状态与描述符兼容路由保留 | C++ 源码合同 + 工厂类型断言 + local/S3 存储行为回归；不删除 readiness、持久 Blob GC 或描述符兼容路由 |
