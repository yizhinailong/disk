...
| `RuntimeConfig_test.cpp` | 最终配置只允许一个固定 `from_header=x-real-ip`、`attribute_key=disk-client-ip` 的 `RealIpResolver`，其 `trust_ips` 为 0-32 个严格点分十进制 IPv4 或规范 IPv4 CIDR 字符串；环境覆盖仍要求 1-32 项。拒绝 resolver 缺失/重复/畸形/重定向，以及结构错误、IPv6、端口、空白、前导零、`/0`、越界前缀和带主机位网络，异常不回显值，且不提供公开的半管线解析入口 |
