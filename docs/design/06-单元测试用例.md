...
| `ProcessRuntime_test.cpp` / `StorageWorkerRuntime_test.cpp` / `test_secure_local_staging_cutoff.py` / `test_worker_drain_takeover.py` / `test_distributed_topology.py` | 实例注册前 bootstrap 使用空 instance/所有权与固定失败摘要；drain 幂等停止后续 poll；进程与 Worker 运行时事件使用实际 instance、各自固定 operation 和空请求/任务所有权，失败消息不泄露底层错误；真实安全模式 API/local staging 在监听前以退出码 1 拒绝并输出合规 bootstrap 摘要；真实 Worker A 收到 SIGTERM 后退出 readiness、不认领新哨兵，Worker 与进程 draining/deadline 事件逐行满足结构化合同，超时退出后保留租约并由 B 自然接管；随后 B/C 独占竞争 | C++ 源码合同 + 内存 NDJSON/运行时单测 + 真实启动拒绝 + 唯一临时 PostgreSQL/三个真实 Worker 进程（任一竞争阶段同时两个）+ Compose 静态合同；不直接更新任务状态或租约 |
