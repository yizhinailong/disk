...
| `StorageFactory_test.cpp` / `ProcessRuntime_test.cpp` | 工厂 bundle 与 `StorageMgr` 直接使用 `UploadStagingStorage`，ApplicationContext/上传服务只接收暂存和 Blob 能力；仓库不存在空 `IFileStorage`、`GetStorage()`、无用透传成员或三个 manager 静态初始化探针；`ProcessRuntimeState` 实例初始化状态与 local/S3 concrete adapter 暂存能力保留 | C++ 源码合同 + 工厂类型断言；不删除 readiness 状态或描述符兼容路由 |
