...
| `RuntimeConfig_test.cpp` | `DISK_TRUSTED_PROXY_CIDRS` 只接受 1-32 个严格点分十进制 IPv4 或规范 IPv4 CIDR 字符串，精确覆盖唯一 `RealIpResolver.config.trust_ips`；拒绝结构错误、IPv6、端口、空白、前导零、`/0`、越界前缀和带主机位网络，异常不回显值，且不提供公开的半管线解析入口 |
