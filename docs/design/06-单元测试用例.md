...
| `LogHelper_test.cpp` | `INFO` 阈值丢弃高频请求明细与成功摘要、完整保留失败摘要；临时 `DEBUG` 阈值恢复成功明细，策略接口不做概率抽样 | 内存日志 sink 单测；不写文件、不启动 HTTP 服务 |
...
| `S3ObjectStorage_test.cpp` / `test_s3_app_flow.py` | key 规范化；不可变 Put；Head 元数据一致性；按索引流式组装；全文件大小/MD5/SHA-256 校验；范围读取不落本地文件 | 内存 S3 client 单测 + 目标 S3/MinIO 环境门控应用流 |
