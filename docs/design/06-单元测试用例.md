...
| `QuotaService_test.cpp` / `UploadTaskRepository_test.cpp` / `StorageReconciliationService_test.cpp` / `test_safety_upload_invariants.py` / `test_safety_content_quota.py` | 8 个活跃 standalone/transaction-client 配额写入入口按值接收默认空上下文，15 条操作事件全部使用调用方上下文且构造事件保持空请求/任务关联；编译期/源码合同拒绝两个无调用 `GetReconciliation` 形状、吞错的 `AdjustUsedStorage` 与两个 `ReleaseReservedStorage` facade、专属结构和 SQL 回归，保留 UploadLifecycle/FileMutation 与 Trash 的受检查生产调用；非秒传 init 必须把预留与任务插入放入同一 `TransactionRunner`，仓储 `Create` 必须使用调用方 transaction client，真实 PostgreSQL trigger 拒绝任务插入时任务、配额和无归属预留量均不变且移除 trigger 后重试成功；永久删除继续显式传播 `AdjustUsedStorageChecked` 失败；`StorageReconciliationService` 继续分页检测并持久化/消解 used/reserved 配额 finding；真实 quota 拒绝保持 HTTP 400/`50004`、零新任务/预留和关联日志合同；日志不含 user/bytes/delta、SQL、连接、domain error、数据库异常、JWT/凭据，不从参数、行数或错误推断字段 | 编译期/源码合同 GoogleTest + 真实 HTTP/PostgreSQL/故障注入/受管进程 NDJSON |
