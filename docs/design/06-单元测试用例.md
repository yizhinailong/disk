...
| `FolderRepository_test.cpp` / `FolderLogContext_test.cpp` / `test_safety_content_quota.py` | FolderRepository 全部 12 个活跃操作显式接收 standalone client 或当前 transaction，编译期合同要求仓储可默认构造且为空类型，源码合同拒绝无读取 `m_db_client`/注入构造、utility/repository 单项 `FetchFolderDeletePlan`、无调用 `IncrementItemCount` 与 FolderService 吞错 `IncrementParentItemCount` 回归；嵌套创建在同一 `TransactionRunner` 中检查父目录计数更新、读取稳定父位置并执行 `ON CONFLICT ... DO NOTHING RETURNING`，trigger 拒绝父计数时子目录和计数均不变，移除后重试收敛；8 个并发同名请求只有一个成功，其余稳定返回 `409/50010`，行和计数只增加一次；既有 6 个生产构造点、FolderService 单项子树读取、FileMutation/Trash 三条批量删除计划路径、定位与删除日志上下文合同继续保留 | 编译期/源码合同 GoogleTest + 真实 HTTP/PostgreSQL/并发/故障注入/受管进程 NDJSON |
