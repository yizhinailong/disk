...
| `ProcessRuntime_test.cpp` / `StorageWorkerRuntime_test.cpp` / `test_worker_drain_takeover.py` / `test_distributed_topology.py` | drain 幂等停止后续 poll；进程与 Worker 运行时事件使用实际 instance、各自固定 operation 和空请求/任务所有权，失败消息不泄露底层错误；真实 Worker A 持有被行锁阻塞的任务时收到 SIGTERM，先退出 readiness 且不认领新哨兵，Worker 与进程 draining/deadline 事件逐行满足结构化合同，超时退出后保留租约；Worker B 先处理哨兵，再严格晚于 PostgreSQL 租约截止接管目标；随后 B/C 同时认领两个独立受阻 Blob GC，owner 必须分离且释放后各一次成功；Compose 终止宽限大于应用 drain | C++ 源码合同 + 内存 NDJSON/运行时单测 + 唯一临时 PostgreSQL/三个真实 Worker 进程（任一竞争阶段同时两个）+ Compose 静态合同；不直接更新任务状态或租约 |
