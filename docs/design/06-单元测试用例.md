...
| `test_final_blob_migration.py` | manifest 在默认只读 DB 会话和无 S3 凭据下输出 content ID/旧 locator/相对源路径/大小/MD5/SHA-256/目标 key；命令前后 DB 与源 Blob 快照不变、S3 请求为零、无 checkpoint；`0600` 原子发布不覆盖已有清单，末行故障不留半成品；后续 copy/cutover/rollback 保持既有完整迁移回归 | 真实唯一临时 PostgreSQL + local 源目录 + 可计数受控 S3 HTTP；外部 bucket 无访问，只读 manifest 证据去敏 |
