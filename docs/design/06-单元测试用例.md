...
| `ScheduledTasks_test.cpp` / `ScheduledTasks.hpp` | 周期任务播种执行阶段 `SeedOnce` 只由私有事件循环触发器调用，必须保持私有；进程只公开初始化、注册、停止与 drain 观察生命周期 | C++23 concept 编译期公开面合同；不改变首次/周期触发、去重、角色归属、错误日志或关停语义 |
