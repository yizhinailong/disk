...
| FolderRepository | test/services/FolderRepository_test.cpp | ✅ 已实现（含共享文件持久化日志上下文合同） |
...
| `TokenServiceLogContext_test.cpp` / `AuthLogContext_test.cpp` / `AuthFilterLogContext_test.cpp` / `ShareLogContext_test.cpp` / `test_safety_upload_invariants.py` | access/refresh/share token 生成、验证、refresh CAS、撤销解析和撤销拒绝事件显式按值接收上下文；AuthService、JWT/Share 过滤器和 ShareService 调用点均传入既有上下文，CPU pool 捕获值副本；内存 sink 覆盖 auth/share 成功、畸形 token、撤销解析失败与默认空调用，进程日志保持空关联；真实 refresh 验证计时及带畸形 owner/visitor token 的拒绝日志对账 request/instance/operation 与四个空所有权字段，并扫描原始令牌、头值、密钥和密码排除；claim/签名/TTL/Redis/CAS/撤销/错误响应不变，未使用 TTL helper/Redis client 字段不存在 | 源码合同 + 内存结构化日志 sink GoogleTest + 真实 HTTP/Redis/受管进程 NDJSON |
| `RedisServiceLogContext_test.cpp` / `TokenServiceLogContext_test.cpp` / 七类 `*RateLimitLogContext_test.cpp` / `HealthService_test.cpp` / `test_safety_upload_invariants.py` | 13 个 Redis 命令 API 按值接收可选上下文，23 条命令日志全部使用调用方上下文，初始化保持空关联；源码合同覆盖 TokenService、Auth/Share、FileListCache/FileQuery、七类限流、管理员状态与 readiness 调用点，内存 sink + 真实 Redis 覆盖 EXISTS/EXPIRE/INCR/INCRBY/CAS/固定窗口成功、命令失败和默认空调用；日志不含 key/value/Lua/异常文本/token hash/JTI/分享码/IP/文件列表内容/endpoint/凭据，不从输入推断所有权；真实登录以非整数限流值触发 Redis fail-open，核对 error 与响应关联且登录成功；命令/TTL/事务/Lua/CAS/指标/Result/故障策略不变 | 源码合同 + 内存结构化日志 sink + 真实 Redis GoogleTest + 真实 HTTP/Redis/受管进程 NDJSON |
| `QuotaService_test.cpp` / `test_safety_upload_invariants.py` | 13 个 standalone/transaction-client 配额入口按值接收默认空上下文，18 条操作事件全部使用调用方上下文且构造事件保持空关联；源码合同覆盖内部 overload 与 UploadLifecycle/FileMutation/Trash 生产调用，删除四个无调用私有转发 helper；真实 upload init 在 quota=used+reserved 时保持 HTTP 400/`50004`、零新任务/预留，helper 与上层固定拒绝事件核对 request/instance/`upload_init` 及四个空所有权字段；日志不含 user/bytes/delta、SQL、连接、domain error、数据库异常、JWT/凭据，不从参数、行数或错误推断字段；SQL、条件更新、事务和错误语义不变 | 源码合同 GoogleTest + 真实 HTTP/PostgreSQL/受管进程 NDJSON |
| `FolderRepository_test.cpp` / `test_safety_content_quota.py` | FolderRepository 定位与 FileServiceUtils 定位转发/trash 插入/file delete/folder delete 共 5 个共享入口按值接收默认空上下文，TrashService 转发和 UploadLifecycle/FileMutation/Trash 调用点保留原上下文；4 条数据库失败事件固定、无 ID/name/path/SQL/异常正文或字段推断；三个真实 PostgreSQL trigger 失败分别核对 request/instance/`file_mutation` 与四个空所有权字段、无空 request 重复，并排除 4 条直接 application message 中的异常正文，同时保持 HTTP 500/`10006`、事务回滚、active/trash/share/ref_count/quota/Blob 不变量；SQL、affected-row、返回值和公开响应不变 | 源码合同 GoogleTest + 真实 HTTP/PostgreSQL/受管进程 NDJSON |
| `ContentService_test.cpp` / `test_safety_content_quota.py` | 8 个公开查找/引用获取/单批增减/Blob GC 入队入口按值接收默认空上下文，内部 reference gate 及 UploadLifecycle/FileMutation/Share/Trash 全部保留原上下文；10 条操作失败事件固定，无 content/file/user ID、hash、size/delta、path/MIME、batch/affected rows、GC payload、SQL、domain error/异常正文或字段推断，构造事件保持空关联；真实 `file_contents` UPDATE trigger 使秒传引用递增失败，核对 HTTP 500/`10006`、request/instance/`upload_init`、四个空所有权字段、无空 request 重复/application 异常正文，并保持无目标文件/任务、ref_count、quota、内容行和 Blob 不变；SQL、行锁、reference gate、GC 入队、可选值/集合/Result 和公开响应不变 | 源码合同 GoogleTest + 真实 HTTP/PostgreSQL/受管进程 NDJSON |
