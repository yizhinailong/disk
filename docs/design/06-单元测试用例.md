...
| UploadStateMachine | test/services/UploadStateMachine_test.cpp | ✅ 已实现（生产状态解析、finalize/cancel 决策与重放） |
...
| 测试区域 | 必须证明的行为 | 测试边界 |
|----------|----------------|----------|
| `UploadStateMachine_test.cpp` / `UploadTaskRepository_test.cpp` | 只保留有生产调用的状态解析/诊断/终态与 finalize/cancel 请求决策；`Completed` 重放原结果，`Cancelled/Expired/Failed` 结果稳定；源码合同拒绝测试专用通用 transition、内存租约和 complete/cancel/expire 兼容 guard 回归，并锁定真实 PostgreSQL 条件写 | 无数据库的生产决策单测 + 源码合同 |
