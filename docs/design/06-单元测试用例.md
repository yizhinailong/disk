...
| RuntimeConfig | test/utils/RuntimeConfig_test.cpp | ✅ 已实现（含环境覆盖类型/范围、PostgreSQL `connection_number` 精确映射、唯一 `default` 主库客户端门禁，以及编译期拒绝独立 `RuntimeConfig::LoadFile` 公开入口） |
