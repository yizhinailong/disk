...
| ConfigMgr | test/utils/ConfigMgr_test.cpp | ✅ 已实现（含六项分享限流配置） |
...
| StorageWorker | test/services/StorageWorker_test.cpp | ⏳ ADR-002 待实现 |
| ProcessRuntime | test/services/ProcessRuntime_test.cpp | ⏳ ADR-002 待实现 |
| HealthService | test/services/HealthService_test.cpp | ⏳ ADR-002 待实现 |
...
| `StorageJobRepository_test.cpp` | 去重键防止重复逻辑任务；`SKIP LOCKED` 不重复认领；租约续租与过期接管；指数退避；超过上限进入 dead letter | 仓储单测 + 真实 PostgreSQL 集成 |
| `StorageWorker_test.cpp` | 角色门控、并发上限、优雅退出停止认领、对象不存在视为清理成功、临时错误重试、永久错误死信 | 可注入时钟和存储适配器 |
