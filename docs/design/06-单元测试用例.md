...
| `StorageFactory_test.cpp` | 工厂 bundle 与 `StorageMgr` 直接使用 `UploadStagingStorage`，ApplicationContext/上传服务只接收暂存和 Blob 能力；仓库不存在空 `IFileStorage` 接口、`GetStorage()` 或无用透传成员；local/S3 concrete adapter 仍实现暂存能力 | C++ 源码合同 + 工厂类型断言；不删除描述符兼容路由 |
