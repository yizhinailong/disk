...
| FileRepository | test/services/FileRepository_test.cpp | ✅ 已实现（含事务原语与死入口回归合同） |
...
| `RuntimeConfig_test.cpp` / `ConfigMgr_test.cpp` / `test_auth_cluster_consistency.py` / `test_redis_session_persistence.py` | 源码合同拒绝无调用 `ConfigMgr::GetDatabasePassword`/`GetRedisPassword` 凭据 getter 回归；启动环境覆盖仍把数据库/Redis 密码写入 Drogon 客户端配置，安全模式继续 fail closed，真实 PostgreSQL/Redis 连接与跨实例会话保持不变 | 源码合同 + 配置单测 + 真实 PostgreSQL/Redis 集成回归 |
...
| `FileRepository_test.cpp` / `test_safety_move_copy_path.py` | 文件仓储保留按 ID/用户归属读取、批量读取、重命名、位置与逐文件路径更新原语；源码合同拒绝无调用 `UpdateDescendantFilePathsForFolderMove` 及专属路径前缀 SQL 回归，正向锁定 `FolderService` 重命名与 `FileMutationService` 移动在事务中逐文件调用 `UpdateFilePath`；真实嵌套文件夹移动继续对账根/后代文件夹和后代文件路径 | 编译期/源码合同 GoogleTest + 真实 HTTP/PostgreSQL 安全网；不改变移动事务、路径结果或 item count |
