...
| `ShareLogContext_test.cpp` / `ShareService.*` | 仅供 Create 使用的 8 位字母数字分享码生成和仅供 List 使用的状态筛选映射必须留在 `ShareService.cpp` 匿名命名空间，头文件不得声明对应类成员；Create/List 各保留一个局部调用，all/未知值无过滤，active/expired/cancelled 映射现有枚举。日期、链接、有效性和密码等多调用 helper 不纳入本批 | 集中源码内部链接/规则合同 + 分享 DTO/创建/列表/管理真实集成；事务、权限、密码、分页、审计、日志、错误和响应不变 |
