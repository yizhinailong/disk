...
| ShareAuthFilter | test/filters/ShareAuthFilter_test.cpp | ✅ 已实现；JTI 属性时序覆盖待补 |
| ShareRateLimitFilter | test/filters/ShareRateLimitFilter_test.cpp | ⏳ 待实现（访问、浏览、下载、响应与 fail-open） |
| FilterOwnership | test/filters/FilterOwnership_test.cpp | ✅ 已实现；三桶归属和顺序覆盖待补 |
...
| ConfigMgr | test/utils/ConfigMgr_test.cpp | ✅ 已实现；六项分享限流配置覆盖待补 |
| FileHashUtil | test/utils/FileHashUtil_test.cpp | ✅ 已实现 |
| RedisKeyPrefix | test/utils/RedisKeyPrefix_test.cpp | ✅ 已实现；三类分享固定窗口键覆盖待补 |
