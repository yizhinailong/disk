...
| `ShareLogContext_test.cpp` / `ShareService.*` | 七个无实例状态 helper 必须留在 `ShareService.cpp` 匿名命名空间且不得由头文件声明：Create/List 各自的分享码/状态筛选保留既有规则；剩余 expired/active/password/date/link helper 保留调用数量，按持久状态与过期时间判定有效性、无密码通过/有密码验证 hash、Trantor 可重入且禁用微秒的本地 `%Y-%m-%d %H:%M:%S` 日期和 `/s/<code>` 链接；日期 helper 不得回归 `std::localtime()`/手工 `std::put_time()`/`toDbStringLocal()` | 集中源码内部链接/规则合同 + 分享 DTO/创建/访问/列表/管理/密码/审计真实集成；事务、权限、计数、Redis 限流、日志、错误和响应不变 |
