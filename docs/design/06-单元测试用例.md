...
| `FileRepository_test.cpp` / `FileServiceUtils.*` | 共享 `ExtractFileExtension` 覆盖无点、末尾点、单点、多点和大小写原样后缀；上传秒传/完成两个元数据写入点与文件重命名唯一写入点必须直接调用该 helper，两个 service 不保留本地同名实现。回收站恢复冲突名解析保持专用 | 直接行为 GoogleTest + 集中源码合同；文件名校验、名称、路径、事务、错误、响应和回收站恢复语义不变 |
