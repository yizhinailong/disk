...
- 认证服务在使用连接 peer 地址构造登录限流 key、更新 `last_login_ip` 和写 logout 操作日志前只归一化一次，三处必须复用同一无端口值。`VARCHAR(45)` 容纳最长 IPv6 地址文本，不容纳 `[IPv6]:port`；代理转发头只有在独立受信代理配置完成后才能替换连接 peer。
