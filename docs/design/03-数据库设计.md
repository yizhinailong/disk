...
完成上传获取内容引用时固定使用单条 `INSERT ... ON CONFLICT (hash_md5, hash_sha256)
DO UPDATE SET ref_count = file_contents.ref_count + 1 ... RETURNING`。冲突分支必须保留已有行的
`storage_path`、MIME 和创建时间，仅在已有 `size` 与候选值一致时增加引用；条件不匹配且没有
`RETURNING` 行时按完整性错误回滚整个完成事务。“先查再插”只能用于避免不必要的对象提升，
不能作为并发去重的正确性边界。
...
状态值：`0 Pending`、`1 Running`、`2 Retry`、`3 Succeeded`、`4 DeadLetter`。`payload` 只保存任务参数，不保存密钥、令牌或文件正文。相同逻辑副作用使用稳定 `dedupe_key`，例如 `staging-cleanup:{upload_id}` 或 `blob-gc:{content_id}:{state_version}`。
...
- **并行处理语义**：批量彻底删除在事务内原子递减 `ref_count` 并写入去重的 Blob GC 任务。Worker 删除前再次确认内容无引用；本地开发存储和 S3/MinIO 使用相同的零引用校验与幂等删除约束。
