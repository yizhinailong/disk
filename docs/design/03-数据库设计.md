...
- **storage_reserved**（预占用存储）：
  ...
  ```
  上传预留可由 `upload_tasks.status = 0` 的 `reserved_bytes` 对账；复制预留是应用事务外层的短生命周期预留，没有独立持久任务行，因此正常请求结束后必须全部提交到 `storage_used` 或释放。对账结果中超出进行中上传任务总额的部分，应视为正在执行的复制预留或需要告警的孤儿预留。
...
- **并行处理语义**：批量彻底删除时，`ref_count` 递减和最终 blob 清理由当前 `BlobStore` 的工作队列并行执行（最大并发 4）。本地文件系统和 S3/MinIO 使用相同的零引用校验与幂等删除约束；`ref_count` 更新在数据库层保证原子性。
...
- **0 → 3（进行中 → 已过期）**：超过 `expires_at` 时间未完成，在同一数据库事务中标记任务过期、释放 `storage_reserved` 并删除 `upload_task_chunks`；事务提交后幂等清理临时文件
