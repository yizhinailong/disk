...
批量文件复制也复用 `file-name` 名称锁。事务外冲突查询只能作为减少工作量的快照，不能作为并发裁决；每个显式文件批次必须在同一事务中对候选目标名称去重排序、依次获取名称锁并重新查询占用，再基于锁内剩余项增加 `file_contents.ref_count`、插入文件元数据和把预留配额转为已用配额。锁内发现的冲突项必须在该事务中释放对应预留配额且不修改引用计数；唯一约束仅保留最终不变量，不作为正常跳过分支。
