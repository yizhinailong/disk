...
批量文件夹移动也复用 `folder-name` 名称锁。事务必须先读取并确定顶层移动根，再收集实际跨目录项的
目标名称、去重排序并依次获取名称锁，最后在锁内重新查询目标目录占用并更新根位置及完整子树路径。
已占用名称按批量部分成功语义跳过，输家不得更新根、后代路径或父目录 `item_count`；根位置、子树路径
和源/目标计数的受影响行必须检查，唯一约束仅保留最终不变量，不作为正常跳过或 HTTP 500 分支。
