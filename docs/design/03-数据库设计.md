...
| status | TINYINT | 账户状态：0-禁用, 1-正常, 2-锁定 |
...
| finalized_at | DATETIME | 任务完成或失败时间 |
...
| uploaded_at | DATETIME | 分片上传完成时间 |

**并发安全说明：**

- 使用 `PRIMARY KEY(task_id, chunk_index)` 确保同一任务的同一分片只能上传一次
- 应用层使用 `INSERT IGNORE` 语句插入分片记录，避免并发上传时的写冲突
...
AND finalized_at < DATE_SUB(NOW(), INTERVAL 7 DAY)
LIMIT 1000;
