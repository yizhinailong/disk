...
| last_login_ip | VARCHAR(45) | 最后登录连接 peer IP，仅保存去除 TCP 源端口后的 IPv4/IPv6 文本 |
...
- `status=0` 为管理员禁用，`status=2 + locked_until IS NULL` 为管理员锁定；管理员状态变更同时把 `login_attempts` 置 0、`locked_until` 置 NULL。
- 自动防暴力破解保持 `status=1`。每次密码失败通过单条 PostgreSQL `UPDATE ... RETURNING` 原子增加 `login_attempts`；第 5 次设置 `locked_until=NOW()+INTERVAL '15 minutes'`，已经处于有效锁定的并发请求不得继续增加次数或延长截止时间。
- 账户可用性、登录和 refresh 的临时锁定判断只使用 PostgreSQL `NOW()`。正确登录必须先以条件更新确认账户仍可用，再原子写 `last_login_at=NOW()` 并清零失败状态，成功后才允许签发令牌。
- 兼容旧实现曾写入的自动锁定行：`status=2 + locked_until IS NOT NULL + locked_until<=NOW()` 可在正确认证的条件更新中一次性规范化为 `status=1`；`status=2 + locked_until IS NULL` 不得自动解锁。
- 认证服务在构造登录限流 key、更新 `last_login_ip` 和写 logout 操作日志前只归一化一次，三处必须复用同一无端口值。`VARCHAR(45)` 容纳最长 IPv6 地址文本，不容纳 `[IPv6]:port`。统一解析边界只接受 Drogon `RealIpResolver` 写入的 request attribute：transport peer 必须命中显式受信代理 IPv4/CIDR allowlist，否则持久化连接 peer；数据库层不解析或裁决任何转发头。
...
上传秒传/完成与文件重命名写入 `files.extension` 时统一调用 `FileServiceUtils::ExtractFileExtension`，按最后一个 `.` 提取后缀；无点或点位于末尾时写入空字符串，其余后缀字节保持原样。该共享函数不承担文件名校验、大小写归一化或恢复冲突命名；请求仍先走现有 DTO 文件名规则，回收站恢复继续使用其识别 `name (n).ext` 的专用拆分逻辑。
...
取消、过期和批量过期查询必须在 SQL 内使用 `COALESCE(staging_prefix, temp_path) AS staging_prefix` 构造统一清理描述符。`UploadTaskCleanupRecord` 只保存该统一描述符，不再额外投影或保存无人读取的原始 `temp_path`；这不删除数据库列、不改变旧 local 路径解释，也不取消任何兼容回退。
...
`finalize_attempts` 是数据库持久诊断计数，认领与接管 CAS 必须继续原子递增，故障测试和运维查询直接读取该列。完成流程的 C++ 认领结果不得重复投影或保存该计数，只返回调用方实际消费的 disposition、`state_version` 和完成重放所需的 `completed_file_id`。
...
`last_error` 是有界运维摘要，不是下游诊断正文存档。当前 Worker 新写入时只允许任务合同的固定校验文本、固定配置/状态摘要或固定任务操作失败摘要；不得复制 `ErrorInfo.message`、数据库/标准异常正文、SQL、连接信息、endpoint、对象定位符、payload 值或凭据。临时/永久分类继续由错误码和任务规则决定，仓储仍以 2048 字符作为最终长度保护；本合同不授权自动改写历史行。
...
文件夹软删除还必须按锁后 `top_level_folder_ids` 对每个删除根的外部非根 `parent_id` 聚合 `-1`，与未被文件夹子树覆盖的显式文件来源增量合并后按父目录 ID 升序执行。被另一个删除根覆盖的后代和子树内部目录不得重复扣减；根目录不更新虚拟父计数。任一 `ApplyItemCountDelta` 影响 0 行时，当前事务的回收站快照、分享清理、活跃文件/目录删除和全部计数变化必须整体回滚。
...
文件夹恢复必须对 `trash.id + user_id` 使用相同的行锁与唯一消费协议。每个根名称候选使用短事务：锁定回收站项后解析锁后 `folder_tree` 快照，先取得 `folder-name:<user_id>:<target_parent_id>:<candidate>` transaction advisory lock，再锁定仍存在的非根目标父目录并复查名称占用。成功事务按快照父子关系重建根、后代目录和文件；历史兼容记录没有有效快照时只插入 `item_count = 0` 的空根。完整重建后对最终非根父目录执行 `ApplyItemCountDelta(+1)`，最后以用户和 `item_type = 'folder'` 精确删除回收站行并要求影响 1 行。节点插入、父计数或回收站消费任一步失败都必须回滚全部活跃节点和计数；同一回收站项不得重复恢复。恢复复用软删除期间保留的内容引用与配额，不增加 `file_contents.ref_count` 或用户用量。

显式单文件软删除和恢复必须对非根父目录维护对称的直属计数。软删除事务按锁后文件模型的 `folder_id` 聚合实际显式文件数量，在回收站记录插入后、活跃文件删除前按父目录升序执行 `ApplyItemCountDelta(-n)`；被请求文件夹子树覆盖的文件不得重复扣减。文件恢复的成功候选事务在活跃文件插入后、回收站行删除前对最终非根目标执行 `ApplyItemCountDelta(+1)`。任一计数更新影响 0 行时，当前事务的回收站、文件、分享和计数写入必须全部回滚；根目录没有虚拟计数行。
...
`uk_shares_code` 是分享码唯一性的最终并发裁决，不允许应用通过“先查后插”替代该约束。创建事务使用 `INSERT ... ON CONFLICT (share_code) DO NOTHING RETURNING *` 原子尝试候选码；冲突时在同一事务中生成新候选，最多尝试 5 个，耗尽后回滚且不写入 `share_files`。冲突目标必须显式限定为 `share_code`，其他约束和数据库错误继续作为创建失败传播。
...
`details` 只允许保存 9.4.4 的事件字段。不得保存访问密码、密码哈希、原始 Share Token 或认证请求头。`ip_address`、`user_agent` 和 `share_code` 属于审计追踪数据，统一随操作日志保留 90 天，并由 7.3 节的分批清理语句删除；不得另建无期限副本。认证 logout 的 `ip_address` 使用与登录限流、`users.last_login_ip` 相同的无端口连接 peer IP，避免把临时源端口作为审计身份或写入超过 45 字节的 `[IPv6]:port`。
