...
| `AdminService` | 服务层 | 实现管理员业务逻辑（用户管理、存储统计、系统概览等），返回 `drogon::Task<Result<T>>` |
| `AdminAuthFilter` | 全局过滤器 | 验证请求用户 `role=1`，否则返回 403 Forbidden |
...
- 全局过滤器在 `config.json` 的 `drogon::plugin::GlobalFilters` 中注册，匹配路径 `/api/admin/*`
