...
flowchart TB
    ...
    subgraph Data["数据持久化层"]
        ...
        Redis[("Redis<br/>缓存")]
        BlobStorage[("最终 Blob<br/>本地文件系统或 S3/MinIO")]
        Staging[("本地上传暂存")]
    end
...
| 水平扩展 | **目标能力**：完成 ADR-002 的数据库租约、S3 上传暂存和独立 Worker 后支持无粘性多实例；当前本地上传暂存和进程内单飞仍限制生产级横向扩展 |
| 分层架构 | 控制器 → 服务 → 存储 → 数据访问的清晰分层 |
| 全局和路由过滤器 | JWT 认证、限流、分享认证等通过过滤器链实现 |

### 2.3 分布式目标架构

分布式重构采用“可横向扩容的模块化单体”，不立即拆分微服务。API 与 Worker 使用同一制品、不同运行角色：
...
生产级多实例部署以 ADR-002 为准：至少两个无粘性 API 实例、两个可接管 Worker、PostgreSQL 单写高可用端点、Redis 高可用端点和跨节点共享的 S3/MinIO。API 实例不执行集群级周期任务，本地目录不保存生产上传状态。

在 S3 原生上传暂存、数据库完成租约、持久 Worker 和多实例故障测试全部完成前，单纯启动多个现有二进制不构成受支持的分布式部署。
