...
| role | string | 否 | 角色筛选：admin/normal |
...
| action | string | 是 | 操作类型：disable/enable/lock |

#### 9.2.3 业务规则

1. **禁用用户（disable）**：设置 status = 0，用户无法登录
2. **启用用户（enable）**：设置 status = 1，恢复用户登录能力
3. **锁定用户（lock）**：设置 status = 2，临时锁定账户
4. 禁用不等于删除，属于软删除语义
5. **自我保护规则（AdminCannotModifySelf）**：不能修改自身状态，否则返回 80001 错误
6. **最后管理员保护（AdminCannotDemoteLast）**：不能禁用或锁定最后一个管理员账户，否则返回 80002 错误
...
| 错误场景 | 错误码 | 错误信息 |
|----------|--------|----------|
| 禁止修改自身 | 80001 | 不能修改自身账户状态 |
| 禁止操作最后管理员 | 80002 | 不能禁用或锁定最后一个管理员 |
| 用户不存在 | 80003 | 目标用户不存在 |
...
|--------|------|------|------|
| user_id | integer | 是 | 目标用户 ID |
| role | string | 是 | 目标角色：admin/normal |

#### 9.3.3 业务规则

1. **提升为管理员（promote）**：将用户 role 设为 admin
2. **降级为普通用户（demote）**：将用户 role 设为 normal
3. **自我保护规则（AdminCannotModifySelf）**：不能修改自身角色，否则返回 80001 错误
4. **最后管理员保护（AdminCannotDemoteLast）**：不能将最后一个管理员降级，否则返回 80002 错误
...
| 错误场景 | 错误码 | 错误信息 |
|----------|--------|----------|
| 禁止修改自身 | 80001 | 不能修改自身账户角色 |
| 禁止操作最后管理员 | 80002 | 不能降级最后一个管理员 |
| 用户不存在 | 80003 | 目标用户不存在 |
...
管理员可查看指定用户的所有文件列表，以及平台整体存储使用统计。
...
| folder_id | integer | 否 | 限定文件夹 ID，默认根目录 |
...
1. 仅管理员可访问此接口
2. 查看用户文件时，自动继承该用户的目录结构视图
3. 操作日志记录：action 前缀为 `admin.file.*`

#### 9.4.5 约束条件

- 仅能查看文件元数据，无权限下载或修改文件内容
