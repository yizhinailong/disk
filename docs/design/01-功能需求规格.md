...
5. **地址归一化与代理信任**: 注册、登录和公开分享限流以及认证/分享/存储管理审计统一使用 `ResolveClientIp()`。Drogon `RealIpResolver` 只在 transport peer 命中显式 IPv4/CIDR allowlist 时把 Nginx 覆盖写入的单值 IPv4 `X-Real-IP` 保存为权威 request attribute；allowlist 为空、peer 不受信或 attribute 缺失时必须回退到连接 peer。业务代码不得直接解析 `X-Real-IP` 或可追加的 `X-Forwarded-For`，最终地址不得包含 TCP 源端口。当前 Drogon 1.9.11 的受信 peer 和头部客户端地址解析都只支持 IPv4；IPv6 transport peer 仅允许按未受信直连语义回退，不得宣称代理后的 IPv6 客户端已被区分

**Redis 实现示例**:

```cpp
// 在 AuthService::Login() 开始时检查
auto redis_service = RedisService::GetInstance();

// 使用 RedisService 的 Lua 原子固定窗口操作
const auto ip = ResolveClientIp(request);
const auto rate_key = "rate:login:" + ip;

// 同一脚本内递增，且只在首次递增时设置 5 分钟 TTL
const auto count_result =
    co_await redis_service->IncrWithExpire(rate_key, 300, log_context);
if (!count_result.has_value()) {
    // 限流计数依赖失败按已批准策略 fail open
    co_return ContinueRequest();
}
...
// 如果登录成功，清除计数器
// (在登录成功逻辑中执行)
co_await redis_service->Delete(rate_key, log_context);
```
...
| 回收站接口 | 100 次/分钟/用户 | 用户 ID | 60 秒 |
| 上传接口 | 60 次/分钟/用户 | 用户 ID | 60 秒 |
| 所有者下载接口 | 默认 60 次/分钟/用户 | 用户 ID | 默认 60 秒 |

**回收站通用用户限流 Redis Key 设计**:
```
Key: rate:api:{user_id}:{window_timestamp}
Type: String (计数器)
TTL: 60 秒（1 分钟）
Value: API 请求次数
```
...
1. **路由级拦截**: `RateLimitFilter` 只由 5 条回收站路由声明；上传、下载、文件夹、管理员和分享使用各自的专用限流器
2. **固定窗口算法**: 使用 Redis Lua 封装的原子递增与首次写入过期时间实现 60 秒固定窗口
3. **响应头**: 返回 `X-RateLimit-Limit`、`X-RateLimit-Remaining`、`X-RateLimit-Reset` 头

**实现优先级**: P1（性能和安全功能）
**当前状态**: ✅ 已实现（注册、登录、上传、下载、文件夹、管理员、回收站与分享分域限流）
...
flowchart TD
    subgraph Step1["1. 初始化上传"]
        ...
        A4 -->|"否"| A5
        A5 -->|"是"| A7["返回断点续传信息"]
        A5 -->|"否"| A8["预留配额并创建上传任务<br/>固化 staging backend/prefix"]
    end

    subgraph Step2["2. 分片上传"]
        B1["客户端将文件分割为固定大小的分片<br/>(默认 5MB)"]
        B2["分片可并行发往任意 API"]
        B3["验证 index/size/hash"]
        B4["条件写入不可变 staging 对象"]
        B5["PostgreSQL 条件记录分片描述符"]
    ...
    subgraph Step3["3. 合并完成"]
        C1["完成请求可到达任意 API"]
        C2["PostgreSQL 租约认领并校验分片"]
        C3["有界组装并校验 MD5/SHA-256"]
        C4["提升到内容寻址 final Blob"]
        C5["原子提交文件/ref_count/配额/终态<br/>并入队 staging_cleanup"]
        C6["Worker 幂等清理会话前缀"]
...
1. 检查用户可用配额是否足够
2. 按文件 MD5 查询已有内容（秒传检测）
3. 如果已有内容命中，在同一事务内计费、增加 ref_count 并创建逻辑文件引用
4. 如果同一用户存在相同 MD5 的未过期任务，返回其持久化分片列表（断点续传）
5. 分片大小由服务端配置决定，默认 5 MiB
...
1. 验证分片哈希完整性
2. 支持并行上传，无需按顺序
3. 同一 index/hash/size 的重复分片写入幂等成功；不同内容不得覆盖已有分片
4. 新任务持久化 staging backend/prefix，后续请求与 Worker 始终按任务描述符访问暂存
5. 过期任务和遗留暂存通过 PostgreSQL 持久任务由 Worker 租约认领、重试和接管
...
1. 通过 PostgreSQL 条件更新认领完成租约；有效租约冲突返回稳定错误，已完成重放返回同一文件
2. 按任务持久化的 staging 描述符验证并有界组装全部分片
3. 验证组装结果的大小、MD5 与服务端计算的 SHA-256
4. 将组装结果幂等提升到 SHA-256 内容寻址 final Blob
5. 在一个数据库事务中创建文件/内容引用、结算配额、标记终态并入队唯一 `staging_cleanup`
6. Worker 异步清理暂存；完成响应不依赖请求进程同步删除分片
...
flowchart TD
    subgraph Upload["文件上传时"]
        U1["客户端提交 MD5"]
        U2["按 MD5 查询 file_contents"]
        U3{"已有内容?"}
        U4["原子增加 ref_count<br/>创建逻辑文件引用 (秒传/去重)"]
        U5["上传并由服务端计算 MD5/SHA-256<br/>提升内容寻址 Blob，创建内容与文件记录"]
    ...
    subgraph Delete["文件删除时"]
        D1["普通删除移入回收站<br/>引用与配额保持不变"]
        D2["彻底删除事务减少 ref_count"]
        D3{"引用计数 = 0?"}
        D4["入队唯一 blob_gc 任务"]
        D5["保留共享 Blob"]
        D6["Worker 重新检查零引用<br/>再幂等删除 final Blob"]
...
- **file_contents**：存储内容寻址 Blob 描述符、哈希和引用计数
- **files**：存储文件元数据，通过 content_id 引用内容 Blob
- **ref_count**：逻辑文件/回收站记录对内容 Blob 的引用计数
...
3. 如存在同名文件，自动重命名为 `name (n).ext` 格式（n 从 1 开始递增）
   ...
   - 文件夹同理：`工作` → `工作 (1)` → `工作 (2)`
4. **文件夹子树恢复**：带 `folder_tree` 快照的文件夹回收站项递归恢复完整目录与文件树；历史兼容记录缺少有效快照时仅恢复空根文件夹。根、后代、最终非根父目录计数和回收站消费必须在同一事务内提交或全部回滚
...
1. Worker 为当前 UTC 时间窗播种去重的 `expire_trash` 持久任务，不由 API 实例执行集群级定时器
2. Worker 通过数据库租约分页删除超过 30 天的回收站内容，失败可重试、租约过期可接管
3. 保留天数和执行窗口可配置，清理结果通过任务状态、日志和指标观测
...
- 搜索结果按索引依赖，使用 PostgreSQL GIN 索引与 tsvector 全文检索
...
| 参数名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| share_id | string | 是 | 外部分享 ID（`shares.share_code`） |
