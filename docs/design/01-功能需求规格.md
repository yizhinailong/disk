...
5. **地址归一化**: 认证服务以连接 peer 为当前可信来源，在构造限流 key、写入 `users.last_login_ip` 或记录 logout 审计前统一移除 TCP 源端口；IPv4/IPv6 都只保留地址文本。未启用并配置受信代理解析器时不得直接采信调用方可伪造的 `X-Real-IP`/`X-Forwarded-For`
...
const auto ip = RedisKeyPrefix::ExtractIPOnly(request->getPeerAddr().toIpPort());
