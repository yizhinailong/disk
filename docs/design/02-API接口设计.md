...
2. **命名冲突处理**：如目标位置存在同名文件/文件夹，自动重命名为 `name (n).ext` 格式（n 从 1 开始递增）
   ...
   - 文件夹同理：`工作` → `工作 (1)` → `工作 (2)`
3. **文件恢复原子性**：恢复文件时必须在同一 PostgreSQL 事务内锁定回收站行和目标父目录、确定目标名称、插入活跃文件并删除回收站行。回收站删除未命中时全部回滚；并发恢复同一 `trash_id` 只能有一个成功消费者，不得同时留下活跃文件与原回收站项
4. **V1 限制**：当前版本不支持递归恢复文件夹子树，文件夹类型项目恢复时仅恢复文件夹本身（不包含内部子项）
