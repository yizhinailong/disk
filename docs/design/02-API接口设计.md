...
>
> **并发同名语义**：多个请求把不同目录下的同名文件并发移动到同一目标目录时，恰好一个文件完成移动；其余同名项按既有批量冲突规则跳过，请求仍返回 HTTP 200/业务码 0，且该请求的 `moved_file_count` 与 `moved_count` 不计入冲突项。输家保留原目录与路径，服务端不得暴露 PostgreSQL 唯一约束异常。
