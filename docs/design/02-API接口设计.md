...
- 注册、登录和公开分享限流以及认证/分享/存储管理审计共用统一客户端 IP 解析边界。只有 transport peer 命中 `DISK_TRUSTED_PROXY_CIDRS` 注入的 Drogon `RealIpResolver` allowlist 时，Nginx 覆盖写入的单值 IPv4 `X-Real-IP` 才能替代连接 peer；空 allowlist、非受信 peer 或缺少解析 attribute 时回退连接 peer。业务代码不得直接解析 `X-Real-IP` 或 `X-Forwarded-For`，持久值不得包含 TCP 源端口。当前 Drogon 1.9.11 不支持从该代理头解析 IPv6 客户端地址，参考入口因此只承诺 IPv4 客户端地址隔离。
