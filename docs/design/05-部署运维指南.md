...
| `MYSQL_PASSWORD` | MySQL数据库密码 | 强密码，避免使用默认值 | 仅安全模式 |
...
- 当 `DISK_SECURE_MODE=true` 时，应用启动时还会额外验证 `MYSQL_PASSWORD` 和 `REDIS_PASSWORD`
...
# 执行迁移脚本
mysql -u disk_user -p disk < sql/migrations/V001_forward.sql
```
...
SHOW COLUMNS FROM upload_tasks LIKE 'reserved_bytes';
SHOW COLUMNS FROM upload_tasks LIKE 'finalized_at';
SHOW COLUMNS FROM upload_tasks LIKE 'fail_reason';
-- 预期结果: 3 行结果，显示字段定义

-- 验证 4: trash.content_id 字段已存在
SHOW COLUMNS FROM trash LIKE 'content_id';
-- 预期结果: 1 行结果，显示字段定义
...
SHOW INDEX FROM upload_tasks
WHERE Key_name IN ('idx_upload_tasks_user_status', 'idx_upload_tasks_status_expires');
-- 预期结果: 2 行结果（两个新索引已创建）
```
...
```bash
# 执行回滚脚本
mysql -u disk_user -p disk < sql/migrations/V001_rollback.sql
```
...
```bash
# 从最新备份恢复
gunzip < /data/backups/mysql/disk_$(date +%Y%m%d_%H%M%S).sql.gz | mysql -u disk_user -p disk
```
...
| 字段 | 类型 | 默认值 | 取值范围 | 说明 |
|------|------|--------|----------|------|
| `jwt.secret` | string | - | 最少32字符 | JWT 签名密钥，生产环境必须使用强随机值 |
| `jwt.access_token_expire` | integer | 7200 | >0 | Access Token 有效期（秒） |
| `jwt.refresh_token_expire` | integer | 604800 | >0 | Refresh Token 有效期（秒） |
| `storage.path` | string | - | 有效路径 | 用户文件存储根目录 |
| `storage.temp_path` | string | - | 有效路径 | 临时文件存储目录 |
| `storage.default_quota` | integer | 10737418240 | >0 | 用户默认存储配额（字节） |
| `storage.file_io_threads` | integer | 4 | 2-16 | 本地文件 I/O 线程池大小 |
| `rate_limit.login.requests` | integer | 10 | >0 | 登录接口每周期最大请求数 |
| `rate_limit.login.period` | integer | 60 | >0 | 登录限流周期（秒） |
| `rate_limit.api.requests` | integer | 100 | >0 | API 接口每周期最大请求数 |
| `rate_limit.api.period` | integer | 60 | >0 | API 限流周期（秒） |

> **注意**：`file_io_threads` 控制 `LocalFileStorage` 线程池大小，用于并行执行物理文件 I/O 操作（如 blob 清理）。默认值 4 可满足大多数场景；高并发部署可适当调大，但不宜超过 16。
