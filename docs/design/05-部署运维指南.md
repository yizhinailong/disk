...
baseline 成功率 100%、完成均值 250.565 ms、P99 286.412 ms；canary 成功率 100%，其分片
交替命中不同默认值 API，并由 local-default API 在 425.640 ms 内完成。监控为
92 次 S3 成功调用、0 对象错误、0 过期租约/接管/死信/未解决 finding，10/10
cleanup 成功；证据为 `.sisyphus/evidence/staging-canary-summary.json`（SHA-256
`06a44297e9c3e0cef47d3e4c506c2e7385d9d762c416e5b5a026880321cd37a9`）。该记录仅证明候选制品和灰度
...
`3171b6941f1fc10dbbfc45b0c51f4b6efa449d03ba220dd4b5625168ba2c13d3`）。完整 CTest 为
1372 通过、6 项环境门控跳过、0 失败，总耗时 360.08 秒。该门禁只验证关键阶段；
