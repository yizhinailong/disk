...
进程先读取 `DISK_CONFIG_FILE` 指定的 JSON（未设置时为工作目录下的 `config.json`），再以环境变量覆盖运行时连接、监听和对象存储字段。覆盖值必须完整通过类型和范围校验；变量存在但为空、布尔值不是 `true/false/1/0`、端口或连接池越界时，进程直接启动失败。最终生效配置只传给 Drogon，不写回文件，也不得完整打印到日志。
...
| S3 | `DISK_S3_BUCKET`、`DISK_S3_REGION`、`DISK_S3_ENDPOINT`、`DISK_S3_USE_SSL`、`DISK_S3_FORCE_PATH_STYLE`、`DISK_S3_VERIFY_SSL`、`DISK_S3_OBJECT_PREFIX`、`DISK_S3_STAGING_PREFIX`、`DISK_S3_CONNECT_TIMEOUT_MS`、`DISK_S3_REQUEST_TIMEOUT_MS`、`DISK_S3_MAX_RETRIES`、`DISK_S3_RETRY_BASE_DELAY_MS` | `custom_config.disk.s3.*` |
...
默认连接预算为每个应用进程 8 条 PostgreSQL 连接和 4 条 Redis 连接。双 API + 双 Worker 合计最多 32 条 PostgreSQL、16 条 Redis 应用连接；Compose 的 PostgreSQL `max_connections=100`，预留 8 条迁移/诊断连接后仍有 60 条余量。增加副本前按 `副本数 * 每副本连接数 + 运维预留` 重新计算，禁止只提高副本数。生产端点上限不足时先降低每副本池或评估 PgBouncer；事务池模式启用前必须重新验证 Drogon 事务、prepared statement 和 advisory lock，当前样例不宣称兼容事务池模式。
...
| 告警 | 建议条件 | 处置入口 |
|---|---|---|
| API 可用性 | 5 分钟 5xx 比例 > 1% 或 readiness 连续失败 2 分钟 | 请求日志的 `request_id/instance_id` 与健康组件 |
| API 延迟 | 10 分钟 P99 超过 SLO | `operation` 低基数标签和依赖日志 |
| 任务积压 | 最老 Pending/Retry 年龄 > 300 秒 | `GET /api/admin/storage-jobs` |
...
5. 从任务列表确认状态进入 Pending/Running/Succeeded，并从 `operation_logs` 核对 `admin.storage_job.replay`、操作者、原状态和原因。

重放不改变 `dedupe_key`，也不跳过 Worker 的 payload 校验、Blob 引用复核或租约门禁。冲突、已被其他管理员重放或不再是 DeadLetter 均不得强制覆盖。解除上传完成租约、生成修复任务和对象删除仍需专门的版本化命令，本轮接口不提供任意 SQL/任意 payload 执行能力。
