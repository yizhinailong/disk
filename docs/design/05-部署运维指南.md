...
混跑期间必须满足以下发布门禁：`upload_staging_backend=local`，所有实例共享 PostgreSQL、Redis、`storage_base_path` 与 `temp_upload_path`，负载均衡按 `upload_id` 保持会话亲和。允许把旧格式任务单向交给当前版本接管；当前版本写入带 `object_key` 的新格式分片后不得再路由给 V002 完成。V002 的 Blob 读取接口忽略 `file_contents.storage_path` 并按 MD5 旧布局重建路径，因此当前版本开始完成 SHA-256 Blob 后，`/api/file/download/*` 与 `/api/share/download/*` 的内容下载必须全部路由到当前版本；当前版本负责同时读取历史 MD5 和新 SHA-256 路径。旧实例完全退出前不得开启 S3 新任务或无粘性上传路由。旧实例退出、local 非终态任务归零并完成 staging 对账后，才可继续 ADR-002 的随机路由阶段。
