...
`DISK_TEST_FAULT_INJECTION` 与 `DISK_TEST_PAUSE_AFTER_FINALIZE_CLAIM_UPLOAD_ID` 只属于受控集成测试，不是可部署的运行配置；生产环境不得设置。后端只在非安全模式、前者精确为 `1` 且后者精确匹配当前 upload ID 时，才会在 Finalizing 认领提交后进入测试停顿；安全模式必须无条件忽略该停顿点。
