...
| 变量名 | 说明 | 要求 |
|--------|------|------|
| `JWT_SECRET` | JWT签名密钥 | 最少32字符，建议64字符随机字符串 |
| `MYSQL_PASSWORD` | MySQL数据库密码 | 强密码，避免使用默认值 |
| `REDIS_PASSWORD` | Redis认证密码 | 强密码，与redis.conf中requirepass一致 |
| `DISK_SECURE_MODE` | 安全模式开关 | 设置为`true`或`1`启用严格验证 |

**安全模式行为：**
- 当 `DISK_SECURE_MODE=true` 时，应用启动时会验证所有必需的环境变量
- 如果任何必需变量缺失，应用将拒绝启动并输出错误信息
- 开发环境可以不设置此变量（默认为宽松模式）
