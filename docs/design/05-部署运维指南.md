...
ADR-002 的数据库升级固定使用 `sql/migrations/V003_distributed_upload_forward.sql`，并由独立迁移任务调用 `scripts/migrate-db.sh` 执行。执行器使用 `schema_migrations` 保存脚本 SHA-256，并在同一 PostgreSQL 事务内获取 advisory lock、执行 DDL 和记录版本。API 和 Worker 启动流程只校验 schema 版本，不执行 DDL，避免多副本并发迁移。
...
执行器固定使用 `psql -X -v ON_ERROR_STOP=1 --single-transaction`，并允许在已完成迁移的 schema 上重复执行而不产生第二张表、重复索引或重复约束。若账本中同版本 checksum 与仓库脚本不同，发布立即停止。回滚只允许在新字段仍未承载业务状态、`storage_jobs` 无记录且应用已回退时手工执行 `V003_distributed_upload_rollback.sql`；否则保留 expand schema，回退应用配置并修复数据，不删除恢复所需记录。
...
下例包含 ADR-002 的目标字段；代码落地前，现有版本会忽略其中尚未实现的角色、暂存和 Worker 配置。滚动发布必须在 V003 expand migration 完成后再启用这些字段。
...
| 字段 | 类型 | 默认值 | 取值范围 | 说明 |
|------|------|--------|----------|------|
| `process_role` | string | all | api / worker / all | 进程角色；生产必须显式配置，开发可用 all |
...
| `worker_concurrency` | integer | 4 | >0 | 单 Worker 外部存储任务并发上限 |
...
`instance_id` 在同一进程内跨 `LoadConfig()` 保持稳定。未配置时，每次进程启动生成新的 UUID；容器编排环境应通过 Downward API 或等价机制注入唯一的 `DISK_INSTANCE_ID`。显式 JSON 值、环境变量值或 `upload_finalize_lease_seconds` 超出表中约束时，应用必须启动失败，不能静默回退。
...
final 前缀；失败路径会 Abort 当前 multipart，但 assembled 会保留供租约接管或对账。bucket 生命周期
...
生产环境分别运行 API 与 Worker。同一制品通过 `custom_config.disk.process_role` 选择角色：
...
API 收到终止信号后先从 readiness 摘除并停止接受新请求；Worker 停止认领新任务，给在途任务有限的优雅退出时间。未完成任务不依赖进程内队列，租约到期后由其他 Worker 接管。每个副本的 `instance_id` 必须唯一并保持到进程退出，滚动重启产生新 ID。
