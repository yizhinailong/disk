...
ADR-002 的数据库升级固定使用 `sql/migrations/V003_distributed_upload_forward.sql`，并由独立迁移任务执行。API 和 Worker 启动流程只校验 schema 版本，不执行 DDL，避免多副本并发迁移。
...
前向脚本必须使用 `psql -v ON_ERROR_STOP=1`，并允许在已完成迁移的 schema 上重复执行而不产生第二张表、重复索引或重复约束。回滚只允许在新字段仍未承载业务状态、`storage_jobs` 无未完成任务且应用已回退时执行；否则保留 expand schema，回退应用配置并修复数据，不删除恢复所需记录。
