...
### 2.1 运行验证状态更新（2026-04-25）

| 功能 | 状态 | 说明 |
|------|------|------|
| 注册 | ✅ 已验证无问题 | QML 注册页已接入真实 `/api/auth/register`，成功/失败响应解析和表单校验均已验证 |
| 登录 | ✅ 已验证无问题 | QML 登录页已接入真实 `/api/auth/login`，`admin / Admin123` 本机接口验证返回 `code=0` 且拿到 access token |
