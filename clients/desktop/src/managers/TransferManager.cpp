    ...
    auto TransferManager::VerifyCompletedDownload(const QString& task_id) -> bool {
        ...
        if (expected_size > 0 && actual_size != expected_size) {
            ...
            task.status_detail = QString("下载校验失败：文件大小不匹配（%1/%2 字节）")
                                     ...
                                     .arg(expected_size);
            m_download_model->UpdateTask(task_id, task);
            const QString category = task.auth_domain == "owner" ? QStringLiteral("IntegrityError") : QStringLiteral("VerificationFailed");
            FailDownload(task_id, make_download_error(category, task.status_detail, true, "restart_download"));
            return false;
        }
