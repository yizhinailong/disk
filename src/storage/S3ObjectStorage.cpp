...
namespace disk::storage {
    ...
    auto S3ObjectStorage::PromoteToFinal(const std::filesystem::path& temp_path, const std::string& hash)
        ...
        auto result = co_await RunBlockingS3Task(
            m_worker_queue,
            [client, key, temp_path, object_key]() -> Result<PromoteResult> {
                ...
                if (head_result->exists) {
                    ...
                    return PromoteResult{ .path = object_key, .created = false };
                }
                ...
                return PromoteResult{ .path = object_key, .created = true };
            }
        );
    ...
    auto S3ObjectStorage::SupportsDirectFileResponse() const noexcept -> bool {
        return false;
    }

    auto S3ObjectStorage::OpenForReadRange(
    ...
    auto S3ObjectStorage::DeletePath(const std::filesystem::path& target_path)
        -> drogon::Task<Result<void>> {
        const auto key = ToObjectKey(target_path);
        auto client = m_s3_client;
        auto result = co_await RunBlockingS3Task(
            m_worker_queue,
    ...
    auto S3ObjectStorage::Exists(const std::filesystem::path& target_path)
    ...
    auto S3ObjectStorage::GetFileSize(const std::filesystem::path& target_path)
