...
public:
   ...
   */
  explicit ShareFiles(const drogon::orm::Row &r,
  ...
  /**  For column share_id  */
  /// Get the value of the column share_id, returns the default value if the
  /// column is null
  const int64_t &getValueOfShareId() const noexcept;
  /// Return a shared_ptr object pointing to the column const value, or an empty
  /// shared_ptr object if the column is null
  const std::shared_ptr<int64_t> &getShareId() const noexcept;
  /// Set the value of the column share_id
  void setShareId(const int64_t &pShareId) noexcept;
...
private:
  ...
  std::shared_ptr<int64_t> shareId_;
