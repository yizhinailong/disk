...
namespace disk::file {
    ...
    auto FileMutationService::Delete(DeleteRequest request, uint64_t user_id)
        ...
        std::vector<utils::TrashInsertItem> trash_items;
        ...
        for (const auto folder_id : top_level_folder_ids) {
            ...
            trash_items.push_back({
                ...
                .item_data = utils::BuildFolderSnapshot(plan),
            });
        ...
        try {
            ...
            auto deleted_folder_rows = co_await utils::DeleteFoldersByIds(txn, folder_ids_to_delete);
        ...
        Logger::Info() << "FileMutationService::Delete completed: deleted_count=" << deleted_count
        ...
        /// Invalidate file list cache for unique folder IDs from deleted items
        {
            ...
            std::sort(affected_folder_ids.begin(), affected_folder_ids.end());
            affected_folder_ids.erase(
                std::unique(affected_folder_ids.begin(), affected_folder_ids.end()),
                affected_folder_ids.end()
            );
    ...
    auto FileMutationService::DeleteFilesByIds(
        const drogon::orm::DbClientPtr& client,
