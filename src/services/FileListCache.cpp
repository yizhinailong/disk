...
namespace disk::file {
    ...
    auto FileListCache::GetVersion(
        ...
        auto version_result = co_await redis_service->Get(version_key, log_context);
        if (!version_result) {
        ...
        if (error != std::errc{} || end != encoded_version.data() + encoded_version.size()) {
            Logger::Error(log_context) << "Invalid file list cache version: user_id=" << user_id;
            co_return std::unexpected(ErrorInfo(
                ErrorCode::RedisOperationFailed,
    ...
    auto FileListCache::Invalidate(
        ...
        if (!redis_service) {
            Logger::Error(log_context)
                << "Cannot invalidate file list cache: Redis service is not initialized";
            co_return;
        }

        const auto version_key = disk::redis::RedisKeyPrefix::BuildFileListCacheVersionKey(user_id);
        auto increment_result = co_await redis_service->Incr(version_key, log_context);
        if (!increment_result) {
            Logger::Warn(log_context)
                << "Failed to increment file list cache version: user_id=" << user_id;
            co_return;
        }

        Logger::Debug(log_context) << "File list cache version incremented: user_id=" << user_id
