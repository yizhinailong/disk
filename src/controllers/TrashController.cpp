...
namespace disk::trash {
    ...
    auto TrashController::List(drogon::HttpRequestPtr request)
        ...
        Logger::Info() << "Received get trash list request: " << request->getPeerAddr().toIpPort();
        ...
        if (!parse_result) {
            Logger::Warn() << "Trash list request parameter validation failed: "
        ...
        if (!list_result) {
            Logger::Error() << "Failed to get trash list: " << list_result.error().message;
        ...
        if (!count_result) {
            Logger::Error() << "Failed to get trash count: " << count_result.error().message;
        ...
        Logger::Info() << "Get trash list successful: user_id=" << user_id << ", total=" << *count_result;
    ...
    auto TrashController::Restore(drogon::HttpRequestPtr request)
        ...
        Logger::Info() << "Received batch restore request: " << request->getPeerAddr().toIpPort();
        ...
        if (!parse_result) {
            Logger::Warn() << "Batch restore request parameter validation failed: "
                     << parse_result.error().message;
            co_return Response::Error(parse_result.error());
        }
        ...
        if (!restore_result) {
            Logger::Error() << "Batch restore failed: " << restore_result.error().message;
            co_return Response::Error(restore_result.error());
        }

        /// 步骤 4: 返回成功响应（包含批量操作结果）
        Logger::Info() << "Batch restore completed: user_id=" << user_id
    ...
    auto TrashController::Delete(drogon::HttpRequestPtr request)
        ...
        Logger::Info() << "Received batch permanent delete request: "
        ...
        auto parse_result = TrashBatchRequest::FromRequest(request);
        if (!parse_result) {
            Logger::Warn() << "Batch delete request parameter validation failed: "
                     << parse_result.error().message;
            co_return Response::Error(parse_result.error());
        }
        ...
        if (!delete_result) {
            Logger::Error() << "Batch delete failed: " << delete_result.error().message;
            co_return Response::Error(delete_result.error());
        }

        /// 步骤 4: 返回成功响应（包含批量操作结果）
        Logger::Info() << "Batch delete completed: user_id=" << user_id
    ...
    auto TrashController::DeleteAll(drogon::HttpRequestPtr request)
        ...
        Logger::Info() << "Received empty trash request: " << request->getPeerAddr().toIpPort();
        ...
        if (!delete_all_result) {
            Logger::Error() << "Failed to empty trash: " << delete_all_result.error().message;
            co_return Response::Error(delete_all_result.error());
        }

        /// 步骤 3: 返回成功响应（包含删除统计）
        Logger::Info() << "Empty trash completed: user_id=" << user_id
