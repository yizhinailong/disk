...
namespace disk::folder {
    ...
    auto FolderController::CreateFolder(drogon::HttpRequestPtr request)
        ...
        LOG_INFO << "Received create folder request: " << request->getPeerAddr().toIpPort();

        // 1. 解析并验证请求参数
        auto parse_result = CreateFolderRequest::FromRequest(request);
        if (!parse_result) {
            LOG_WARN << "Create folder request parameter validation failed: "
                     << parse_result.error().message;
            co_return Response::Error(parse_result.error());
        }
        LOG_DEBUG << "Create folder parameter validation passed: name=\"" << parse_result->name
        ...
        if (!result) {
            LOG_ERROR << "Create folder failed: " << result.error().message
                      << " (user_id=" << user_id << ")";
            co_return Response::Error(result.error());
        }

        // 4. 构造响应
        LOG_INFO << "Create folder successful: " << result->name << " (ID: " << result->id
                 << ", user_id: " << user_id << ")";
    ...
    auto FolderController::GetTree(drogon::HttpRequestPtr request)
        ...
        LOG_INFO << "Received get folder tree request: " << request->getPeerAddr().toIpPort();

        // 1. 解析并验证请求参数
        auto parse_result = FolderTreeRequest::FromRequest(request);
        if (!parse_result) {
            LOG_WARN << "Folder tree request parameter validation failed: "
                     << parse_result.error().message;
            co_return Response::Error(parse_result.error());
        }
        ...
        if (!result) {
            LOG_ERROR << "Get folder tree failed: " << result.error().message
                      << " (user_id=" << user_id << ")";
            co_return Response::Error(result.error());
        }

        // 4. 返回成功响应
        LOG_INFO << "Get folder tree successful: user_id=" << user_id
