...
namespace disk::file {
    ...
    auto FileController::InitUpload(drogon::HttpRequestPtr request)
        ...
        auto log_context = disk::controllers::GetRequestLogContext(request, "upload_init");
        ...
        Logger::Debug(log_context)
            << "Initialize upload parameters validated: filename=\"" << parse_result->filename
        ...
        /// 3. 调用 Service 层初始化上传
        auto result = co_await m_upload_service->InitUpload(*parse_result, user_id, log_context);
        if (!result) {
            Logger::Error(log_context)
                << "Initialize upload failed: " << result.error().message
                << " (user_id=" << user_id << ")";
        ...
        if (!result->upload_id.empty()) {
            log_context.upload_id = result->upload_id;
        }
        Logger::Debug(log_context) << "Initialize upload successful: upload_id=" << result->upload_id
    ...
    auto FileController::UploadChunk(drogon::HttpRequestPtr request)
        -> drogon::Task<drogon::HttpResponsePtr> {
        auto log_context = disk::controllers::GetRequestLogContext(request, "upload_chunk");
        ...
        /// 验证必填参数
        if (upload_id.empty()) {
            Logger::Warn(log_context) << "Upload chunk request missing upload_id parameter";
            co_return Response::Error(
                ErrorInfo(ErrorCode::ValidationFailed, "Missing upload_id parameter")
            );
        }
        if (chunk_index_str.empty()) {
            Logger::Warn(log_context) << "Upload chunk request missing chunk_index parameter";
            co_return Response::Error(
                ErrorInfo(ErrorCode::ValidationFailed, "Missing chunk_index parameter")
            );
        }
        if (chunk_hash.empty()) {
            Logger::Warn(log_context) << "Upload chunk request missing chunk_hash parameter";
            co_return Response::Error(
        ...
        try {
            ...
            Logger::Warn(log_context) << "Invalid chunk_index format: " << chunk_index_str;
            co_return Response::Error(
        ...
        if (chunk_hash.length() != 32) {
            Logger::Warn(log_context) << "Invalid chunk_hash format: " << chunk_hash;
        ...
        if (chunk_data.empty()) {
            Logger::Warn(log_context) << "Upload chunk request missing chunk data";
            co_return Response::Error(ErrorInfo(ErrorCode::ValidationFailed, "Missing chunk data"));
        }

        Logger::HighVolumeDetail(log_context)
            << "Upload chunk parameters validated: upload_id=" << upload_id
            << ", chunk_index=" << chunk_index << ", chunk_size=" << chunk_data.size();

        /// 4. 调用 Service 层上传分片
        auto result =
            co_await m_upload_service
                ->UploadChunk(upload_id, chunk_index, chunk_hash, chunk_data, user_id, log_context);
        if (!result) {
            Logger::Error(log_context) << "Upload chunk failed: " << result.error().message
        ...
        Logger::HighVolumeSuccess(log_context)
            << "Upload chunk successful: chunk_index=" << result->chunk_index
            << " (user_id=" << user_id << ", upload_id=" << upload_id << ")";
        co_return Response::Success(result->ToJson());
    }
