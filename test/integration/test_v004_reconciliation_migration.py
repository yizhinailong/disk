...
ROLLBACK = REPO_ROOT / "sql" / "migrations" / "V004_storage_reconciliation_rollback.sql"
...
def run_command(
    command: list[str], database_name: str, *, check: bool = True
...
def verify_schema(database_name: str) -> None:
    ...
    with connect(database_name) as connection:
        ...
        assert scalar(
            ...
            "WHERE indexname = 'idx_storage_reconciliation_unresolved'",
        ) == 1
        ...
        finding = connection.execute(
            "SELECT occurrences, details->>'size' AS size "
...
def verify_upgrade_and_rollback(database_name: str) -> None:
    ...
    blocked = run_sql_file(database_name, ROLLBACK, check=False)
