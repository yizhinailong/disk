...
def verify_drain_state(
    ...
    for case in cases.values():
        ...
        require(
            (unmounted_staging / upload_id / "0.chunk").read_bytes() == case["shadow"],
            f"cleanup crossed into an unmounted directory for {upload_id}",
        )
        require(
            (unmounted_staging / f"{upload_id}.tmp").read_bytes() == case["shadow"],
            f"assembly cleanup crossed into an unmounted directory for {upload_id}",
        )
...
def exercise_local_staging_drain(
    ...
    unmounted_staging = temporary_root / "unmounted-staging"
    for path in (origin_storage, origin_staging, unmounted_staging):
        path.mkdir()

    legacy_port, worker_port, current_api_port = allocate_ports(3)
    ...
    try:
        ...
        for name, case in cases.items():
            ...
            case["shadow"] = f"unmounted-{name}-{run_tag}".encode()
            shadow_dir = unmounted_staging / case["upload_id"]
            shadow_dir.mkdir()
            (shadow_dir / "0.chunk").write_bytes(case["shadow"])
            (unmounted_staging / f"{case['upload_id']}.tmp").write_bytes(case["shadow"])
...
def main() -> int:
    ...
    print(
        "PASS: legacy local staging drained through its original volume by "
        "complete, cancel, and expire paths"
    )
    return 0
