/**
 * @file AdminRateLimitLogContext_test.cpp
 * @brief Administrator rate-limit request correlation contract tests
 *
...
namespace disk::filters {
    namespace {
        ...
        constexpr std::string_view RAW_ACCESS_TOKEN = "raw-admin-access-token";
        constexpr std::string_view RAW_UPLOAD_ID = "private-upload-id";
        constexpr std::string_view RAW_JOB_ID = "private-job-id";
        constexpr std::string_view RAW_SCAN_ID = "private-scan-id";
        constexpr std::string_view RAW_RECOVERY_BODY =
            R"({"dry_run":false,"confirm_upload_id":"private-upload-id","reason":"private-recovery-reason"})";
            ...
            return [](const std::string&, int, disk::utils::LogContext)
                ...
                co_return std::unexpected(
                    ErrorInfo(
                        ...
                        "injected admin rate context failure"
                    )
                );
            };
        }
        ...
        auto CreateRequest(
            ...
            request->setBody(std::string(RAW_RECOVERY_BODY));
        ...
        protected:
            ...
            static auto ExpectContext(
                ...
                EXPECT_EQ(record["instance_id"].asString(), "admin-rate-limit-instance");
                ...
                for (const auto sensitive_value : {
                         ...
                         RAW_SCAN_ID,
                         RAW_RECOVERY_BODY,
                         std::string_view("private-recovery-reason"),
                     }) {
        ...
        TEST(AdminRateLimitLogContextContractTest, EveryDirectEventUsesRequestOwnedContext) {
            const auto source = ReadSourceFile("src/filters/AdminRateLimitFilter.cpp");
        ...
        TEST_F(AdminRateLimitLogContextTest, RoutesPreserveBoundedContextWithoutOwnership) {
            AdminRateLimitFilter failure_filter(MakeFailureCounter());
            EXPECT_EQ(
                drogon::sync_wait(failure_filter.doFilter(CreateRequest(
                    "/api/admin/users",
                    "admin-rate-users-failure"
                ))),
                nullptr
            );
            ExpectContext(
            ...
            AdminRateLimitFilter diagnostics_filter(MakeCountCounter(1));
            EXPECT_EQ(
                drogon::sync_wait(diagnostics_filter.doFilter(CreateRequest(
                    "/api/admin/uploads/" + std::string(RAW_UPLOAD_ID) + "/diagnostics"
                ))),
                nullptr
            );
            ExpectContext(
            ...
            AdminRateLimitFilter rejection_filter(
            ...
            const auto rejection = drogon::sync_wait(
                rejection_filter.doFilter(CreateRequest(
                    "/api/admin/storage-jobs/" + std::string(RAW_JOB_ID) + "/replay",
                    "admin-rate-job-rejection"
                ))
            );
            ...
            AdminRateLimitFilter recovery_filter(MakeCountCounter(1));
            EXPECT_EQ(
                drogon::sync_wait(recovery_filter.doFilter(CreateRequest(
                    "/api/admin/storage-reconciliation/" + std::string(RAW_SCAN_ID) + "/enqueue",
                    "admin-rate-recovery-success"
                ))),
                nullptr
            );
            ExpectContext(
            ...
            AdminRateLimitFilter cleanup_filter(MakeCountCounter(1));
            EXPECT_EQ(
                drogon::sync_wait(cleanup_filter.doFilter(CreateRequest(
                    "/api/admin/maintenance/cleanup/expired",
                    "admin-rate-cleanup-success"
                ))),
                nullptr
            );
            ExpectContext(
