...
// ==================== ListUserFilesRequest Tests ====================

TEST(ListUserFilesRequest, ValidParameters) {
    auto req = CreateQueryRequest({{"page", "1"}, {"page_size", "20"}});
    auto result = ListUserFilesRequest::FromRequest(req);
...
TEST(ListUserFilesRequest, DefaultPagination) {
    auto req = CreateQueryRequest({});
    auto result = ListUserFilesRequest::FromRequest(req);
...
TEST(ListUserFilesRequest, WithFolderId) {
    auto req = CreateQueryRequest({{"folder_id", "123"}});
    auto result = ListUserFilesRequest::FromRequest(req);

    ASSERT_TRUE(result.has_value()) << "folder_id should pass";
    ASSERT_TRUE(result->folder_id.has_value());
    EXPECT_EQ(*result->folder_id, 123);
}

TEST(ListUserFilesRequest, FolderId_Zero) {
    auto req = CreateQueryRequest({{"folder_id", "0"}});
    auto result = ListUserFilesRequest::FromRequest(req);

    ASSERT_TRUE(result.has_value()) << "folder_id=0 (root) should pass";
    ASSERT_TRUE(result->folder_id.has_value());
    EXPECT_EQ(*result->folder_id, 0);
}

TEST(ListUserFilesRequest, InvalidFolderId_NonNumeric) {
    auto req = CreateQueryRequest({{"folder_id", "abc"}});
    auto result = ListUserFilesRequest::FromRequest(req);

    EXPECT_FALSE(result.has_value()) << "folder_id=abc should fail";
    if (!result.has_value()) {
        EXPECT_EQ(result.error().code, ErrorCode::ValidationFailed);
    }
}

TEST(ListUserFilesRequest, InvalidPage_Zero) {
    auto req = CreateQueryRequest({{"page", "0"}});
    auto result = ListUserFilesRequest::FromRequest(req);
...
TEST(ListUserFilesRequest, InvalidPageSize_Zero) {
    auto req = CreateQueryRequest({{"page_size", "0"}});
    auto result = ListUserFilesRequest::FromRequest(req);
...
TEST(ListUserFilesRequest, InvalidPageSize_TooLarge) {
    auto req = CreateQueryRequest({{"page_size", "101"}});
    auto result = ListUserFilesRequest::FromRequest(req);
...
TEST(ListUserFilesRequest, NegativeFolderId_ParsedAsUnsigned) {
    // std::stoull("-1") wraps to UINT64_MAX — DTO treats it as a valid uint64_t
    auto req = CreateQueryRequest({{"folder_id", "-1"}});
    auto result = ListUserFilesRequest::FromRequest(req);

    ASSERT_TRUE(result.has_value()) << "folder_id=-1 is parsed by std::stoull";
    ASSERT_TRUE(result->folder_id.has_value());
    EXPECT_EQ(result->folder_id.value(), std::numeric_limits<uint64_t>::max());
}

TEST(ListUserFilesRequest, FolderId_LargeValue) {
    auto req = CreateQueryRequest({{"folder_id", "9999999999"}});
    auto result = ListUserFilesRequest::FromRequest(req);

    ASSERT_TRUE(result.has_value()) << "Large folder_id should pass";
    ASSERT_TRUE(result->folder_id.has_value());
    EXPECT_EQ(*result->folder_id, 9999999999ULL);
}
...
// ==================== FileDetailResponse Tests ====================

TEST(FileDetailResponse, AllFields) {
    FileDetailResponse response;
    response.id = 1;
    response.name = "test.txt";
    response.type = "file";
    response.size = 1024;
    response.hash = "d41d8cd98f00b204e9800998ecf8427e";
    response.mime_type = "text/plain";
    response.parent_id = 0;
    response.path = "/test.txt";
    response.created_at = "2026-01-01T00:00:00Z";
    response.updated_at = "2026-01-02T00:00:00Z";
    ...
    EXPECT_EQ(json["size"].asUInt64(), 1024ULL);
    EXPECT_EQ(json["hash"].asString(), "d41d8cd98f00b204e9800998ecf8427e");
    EXPECT_EQ(json["mime_type"].asString(), "text/plain");
    EXPECT_EQ(json["parent_id"].asUInt64(), 0ULL);
    EXPECT_EQ(json["path"].asString(), "/test.txt");
    EXPECT_EQ(json["created_at"].asString(), "2026-01-01T00:00:00Z");
    EXPECT_EQ(json["updated_at"].asString(), "2026-01-02T00:00:00Z");
}

TEST(FileDetailResponse, FolderType) {
    FileDetailResponse response;
    response.id = 5;
    ...
    EXPECT_EQ(json["size"].asUInt64(), 0ULL);
}

TEST(FileDetailResponse, EmptyStrings) {
    FileDetailResponse response;
    response.id = 1;
    response.name = "";
    response.type = "";
    ...
    EXPECT_EQ(json["hash"].asString(), "");
    EXPECT_EQ(json["mime_type"].asString(), "");
}

// ==================== FileListResponse Tests ====================

TEST(FileListResponse, WithItems) {
    FileListResponse response;
    response.items.resize(2);
    response.items[0].id = 1;
    response.items[0].name = "file1.txt";
    response.items[0].type = "file";
    response.items[1].id = 2;
    response.items[1].name = "file2.txt";
    response.items[1].type = "file";
    response.pagination.page = 1;
    response.pagination.page_size = 20;
    response.pagination.total = 2;
    response.pagination.total_pages = 1;
    ...
    EXPECT_EQ(json["items"][1]["name"].asString(), "file2.txt");
    EXPECT_TRUE(json.isMember("pagination"));
    EXPECT_EQ(json["pagination"]["total"].asInt(), 2);
}

TEST(FileListResponse, EmptyItems) {
    FileListResponse response;
    response.pagination.page = 1;
    response.pagination.page_size = 20;
    response.pagination.total = 0;
