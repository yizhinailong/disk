...
1. Render all three layers, reject placeholders or non-digest images, perform
   server-side dry-runs, apply the platform layer, verify five eligible nodes in
   two zones, and pass the repository capacity checker for both `2:2` and `4:4`.
...
5. Verify both HPA objects can read CPU and external metrics before accepting
   the release.
